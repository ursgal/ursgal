...
class Meta_UNode(type):
    ...
    def __new__(cls, cls_name, cls_bases, cls_dict):
        new_class = super(Meta_UNode, cls).__new__(cls, cls_name, cls_bases, cls_dict)
        return new_class
    ...
    def __call__(cls, *args, **kwargs):
        ...
        translation_style = initd_klass.META_INFO.get("utranslation_style", None)
        if translation_style is not None:
            Meta_UNode._uparam_mapper.lookup["style_2_engine"][translation_style].add(
                engine
            )
            Meta_UNode._uparam_mapper.lookup["engine_2_style"][
                engine
            ] = translation_style
        else:
            print(
                """META_INFO for engine {0} does not contain a utranslatation_style""".format(
                    engine
                )
            )
            sys.exit(1)
        ...
        alternative_exe_folder = initd_klass.META_INFO.get("uses_unode", None)
        if alternative_exe_folder is not None:
            kwargs["engine_path"] = kwargs["engine_path"].replace(
                engine, alternative_exe_folder
            )
...
class UNode(object, metaclass=Meta_UNode):
    ...
    def determine_common_top_level_folder(self, input_files=None):
        ...
        for input_file in input_files:
            filepath, file_name = os.path.split(input_file)
        ...
        common_top_level_folder = [os.path.abspath(os.sep)]
    ...
    def determine_common_head_tail_part_of_names(self, input_files=None):
        common_head = self.determine_common_name(input_files, mode="head")
        common_tail = self.determine_common_name(input_files, mode="tail")
    ...
    def update_output_json(self):
        ...
        for i_param, i_value in self.io["input"]["params"].items():
            if (
                i_param not in self.io["output"]["params"]
                or self.io["output"]["params"][i_param] != i_value
            ):
    ...
    def dump_json_and_calc_md5(self, stats=None, params=None, calc_md5=True):
        ...
        """
        self.print_info("Preparing json dump", caller="dmpjson")
        if stats is None:
            stats = self.io["output"]["stats"].copy()
        if params is None:
            params = self.io["output"]["params"].copy()
        json_file = os.path.join(
            self.io["output"]["finfo"]["dir"], self.io["output"]["finfo"]["json"]
        )
        output_file = os.path.join(
            self.io["output"]["finfo"]["dir"], self.io["output"]["finfo"]["file"]
        )
        # pprint.pprint( self.io['output'])
        # exit(1)
        if os.path.exists(output_file) and calc_md5:
            self.io["output"]["finfo"]["md5"] = self.calc_md5(output_file)
            # print('''
        ...
        self.print_info("Json dumped. Path: {0}".format(json_file), caller="dmpjson")
        return json_file
    ...
    def _execute(self):
        ...
        assert (
            "command_list" in self.params.keys()
        ), """
  ...
  alternatively, redefine uNode._execute() in the engine class altogether
        ...
        if proc is not None:
            ...
            assert self.execute_return_code in [
                0,
                None,
            ], """
  \n{0} crashed!
  ...
  Inspect the printouts above for possible causes and verify that all input files are valid.
            """.format(
                self.engine, os.path.relpath(self.exe), self.execute_return_code
            )
    ...
    def generate_fasta_database_lookup(self, database):
        if database not in self.lookups["fasta_dbs"].keys():
            ...
            self.print_info("Fasta_db {0}".format(database), caller="Caching")
            current_id = None
            ...
            self.print_info("Done!", caller="Caching")
        ...
        if not history:
            ...
            for history_event in history[::-1]:
                if history_event["engine"] == "merge_csvs_1_0_0":
                    merged_engines = set()
                    for element in self.flatten_list(
                        history_event["history_addon"]["search_engines_of_merged_files"]
                    ):
    ...
    def _group_psms(
        self, input_file, validation_score_field=None, bigger_scores_better=None
    ):
        """
        ...
        if bigger_scores_better is None or validation_score_field is None:
            ...
            assert (
                "multiple engines:" not in last_engine
            ), "Can't convert merged results from multiple different engines."
...
[ WARNING ] automatcally based on the last engine: {},
        ...
        for n, line_dict in enumerate(csv_dict_reader_object):
            assert (
                validation_score_field in line_dict.keys()
            ...
            tmp[line_dict["Spectrum Title"]].append(
                (float(line_dict[validation_score_field]), line_dict)
            )

        for spectrum_title in tmp.keys():
            already_seen = set()

            for score, line_dict in sorted(
                tmp[spectrum_title],
    ...
    def load_json(self, finfo=None, json_path=None):
        ...
        self.compare_json_and_local_ursgal_version(
            history=j_content[3]["history"], json_path=json_path
        )
        return j_content
    ...
    def compare_json_and_local_ursgal_version(self, history, json_path):
        ...
        if json_version != None and json_version != ursgal.__version__:
            self.print_info(
                ("JSON {0} stems from another Ursgal version!").format(
                    os.path.relpath(json_path)
                ),
                caller="/\\",
            )
            self.print_info(
                ("JSON version: {0} - Local version: {1}").format(
                    json_version, ursgal.__version__
                ),
                caller="WARNING!",
            )
            self.print_info(
    ...
    def map_mods(self):
        ...
        for ursgal_index, mod in enumerate(sorted(self.params["modifications"])):
    ...
    [ WARNING ] Continue without modification {0} """.format(
                ...
                unimod_id_list = ursgal.GlobalUnimodMapper.composition2id_list(
                    composition_unimod_style
                )
                mass = ursgal.GlobalUnimodMapper.composition2mass(
                    composition_unimod_style
                )
    ...
    def peptide_regex(self, database, protein_id, peptide):
        ...
        for aa_exception, aa_exception_info in (
            self.meta_unodes["unify_csv_1_0_0"]
            .DEFAULT_PARAMS["aa_exception_dict"]
            .items()
        ):
            # regex will match any character in place of unknown aminoc acid...
            peptide_for_regex = peptide_for_regex.replace(aa_exception, ".")
        ids_matched = []
        if protein_id not in self.lookups["fasta_dbs"][database].keys():
            ...
            for long_protein_id in self.lookups["fasta_dbs"][database].keys():
                if long_protein_id.startswith(protein_id):
                    sequence_to_check = self.lookups["fasta_dbs"][database][
                        long_protein_id
                    ]
        ...
        for protein_id in ids_matched:
            ...
            if buffer_key not in self.lookups["peptide_pos_buffer"].keys():
                ...
                for match in re.finditer(peptide_for_regex, full_sequence):
                    ...
                    pos_aa_protein = (start + 1, stop, pre_aa, post_aa, protein_id)
                    if buffer_key in self.lookups["peptide_pos_buffer"]:
                        self.lookups["peptide_pos_buffer"][buffer_key].append(
                            pos_aa_protein
                        )
                    else:
    ...
    def _preflight(self):
        ...
        if "citation" in self.META_INFO:
            ...
            self.print_info(self.META_INFO["citation"], caller="cite:")
            self.print_info(caller="-----", msg="")
            self.print_info(caller="Citation", msg="")

        self.print_info("Executing preflight sequence ...", caller="PREFLGHT")
    ...
    def _postflight(self):
        self.print_info("Executing postflight sequence ...", caller="POSTFLGHT")
    ...
    def print_execution_time(self, tag=None):
        ...
        if lap > 3600:
            ...
            unit = "seconds"
        msg = "Execution time {0:.3f} {1}".format(lap, unit)
    ...
    def print_info(cls, msg, caller=None):
        ...
        print("[ {0: ^08s} ] {1}".format(caller, msg))

    def print_header(self, header, tag=None, newline=True):
        if tag is not None:
            time_stamp = "({0})".format(
                self.time_point(tag=tag, diff=False, format_time=True, stop=False)
            )
        else:
            time_stamp = ""
        print(
    ...
    def initialize_run(self):
        self.created_tmp_files = []
        self.io = {
            "input": {"finfo": None, "params": None, "stats": None},
            "output": {"finfo": None, "params": None, "stats": None},
        }
    ...
    def fix_md5_and_file_in_json(self, ifile=None, json_path=None):
        ...
        """
        self.print_info("Preparing json fix", caller="fix_md5")
        if ifile is None:
            ifile = json_path.replace(".u.json", "")
            if os.path.exists(ifile) is False:
                raise IOError(
                    "file corresponding to {0} does not exists".format(json_path)
                )
        elif json_path is None:
            json_path = ifile + ".u.json"
            if os.path.exists(json_path) is False:
                raise IOError(
                    "json ({0}) corresponding to {1} does not exists".format(
                        json_path, ifile
                    )
                )
        ...
        with open(json_path, "w") as file_object:
            json.dump(j_content, file_object, sort_keys=True, indent=2)
        self.print_info(
            "New json dumped {0}".format(os.path.basename(json_path)), caller="fix_md5"
        )
    ...
    def run(self, json_path=None):
        ...
        self.print_header(
            "{0} run initialized with {1}".format(
                self.engine, self.io["input"]["finfo"]["file"]
            ),
            tag=tag,
        )
        if self.io["output"]["finfo"]["is_compressed"]:
            ...
            self.print_info(
                'Will compress output {output_file} on the fly ... renamed temporarily params["output_file"]'.format(
                    **self.params
                ),
                caller="run",
            )
        ...
        untranslated_params, translated_params = self.collect_and_translate_params(
            self.params
        )
        ...
        for engine_type in ukb.ENGINE_TYPES.keys():
            if "search_engine" in engine_type:
                is_search_engine = self.META_INFO["engine_type"].get(engine_type, False)
        ...
        map_mods_node_exceptions = ["unify_csv", "ptmshepherd", "ptminer"]
        ...
        for engine_short_name in map_mods_node_exceptions:
            if engine_short_name in self.engine:
                ...
                break
        self.stats["history"] = self.update_history_status(
            status="launching", history=self.stats["history"]
        )

        requires_grouped_psms = self.META_INFO.get("group_psms", False)
        if requires_grouped_psms:
            self.print_info("Grouping PSMs", caller="run")
            self.time_point(tag="group_psms")
            self.params["grouped_psms"] = self._group_psms(
                os.path.join(self.params["input_dir_path"], self.params["input_file"]),
        ...
        self.print_info("Starting engine", caller="run")
        ...
        self.stats["history"] = self.update_history_status(
            status=history_status, history=self.stats["history"]
        )
        # We Use original params for second and final dump
    ...
    def collect_and_translate_params(self, params):
        ...
        for ukey, mDict in self.UNODE_UPARAMS.items():
            ...
            GROUPED_TRANSLATIONS[ukey_t].update({mDict["ukey"]: translated_value})
        translated_params["_grouped_by_translated_key"] = GROUPED_TRANSLATIONS
    ...
    def update_history_status(
        self, engine=None, history=None, status="pending", history_addon=None
    ):
        if history is None:
            raise Exception(
                "Legacy code implicitly updated history ... please change code!"
            )
    ...
    def update_history_with_basic_unode_info(self, history=None, engine=None):
        ...
        if len(history) == 0:
            ...
            if (
                last_history_entry["engine"] == engine
                and last_history_entry["status"] == "complete"
            ):
                init_empty_history_entry = True
            ...
            history.append(
                {
                    ...
                    "META_INFO": self.meta_unodes[
                        engine
                    ].META_INFO,  # NOT self.META_INFO because self is the UController and not the search engine!!!
