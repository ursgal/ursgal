...
class mzidentml_lib_1_6_10(ursgal.UNode):
    ...
    META_INFO = {
        ...
        "citation": "Reisinger F, Krishna R, Ghali F, Rios D, Hermjakob H, "
    ...
    def preflight(self):
        ...
        translations["output_file_incl_path"]["output_file_incl_path"] = self.params[
            "translations"
            ...
            for translated_key, translation_dict in sorted(translations.items()):
                if translated_key == "-Xmx":
                    self.params["command_list"].insert(
                        1,
                        "{0}{1}".format(
                            translated_key, list(translation_dict.values())[0]
                        ),
                    )
                elif translated_key == "mzidentml_function":
                    self.params["command_list"].insert(
