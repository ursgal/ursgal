    ...
    def _execute( self ):
        ...
        for translated_key, translation_dict in translations.items():
            if translated_key in [
                ...
                print('The translated key ', translated_key, ' maps on more than one ukey, but no special rules have been defined')
        ...
        assert all_are_csv is True, "VennDiagram input files all have to be .csv"
        assert len(data) <= 5, '''
