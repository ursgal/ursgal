...
class qvality_2_02(ursgal.UNode):
    ...
    META_INFO = {
        ...
        "citation": "Kall L, Storey JD, Noble WS (2009) QVALITY: non-parametric "
    ...
    def preflight(self):
        ...
        """
        self.params["last_engine"] = self.get_last_search_engine(
        ...
        if (
            self.UNODE_UPARAMS["bigger_scores_better"]["uvalue_style_translation"][
                self.params["last_engine"]
            ]
            is False
        ):
            self.params["command_list"].append(
                "-r"
            )  # False: lower scores are better e.g.OMSSA, scores have to be reversed for qvality
        self.params["command_list"] += [
            self.params["translations"]["target"]["path"],
            self.params["translations"]["decoy"]["path"],
        ]
    ...
    def postflight(self):
        ...
        qvio.close()
        opened_file = open(
            os.path.join(self.params["input_dir_path"], self.params["input_file"]), "r"
        )
        csv_kwargs = {}
        ...
        for line_dict in csv_input:
            ...
            formatted_score = FLOAT_FORMAT_STRING.format(
                float(
                    line_dict[
                        self.UNODE_UPARAMS["validation_score_field"][
    ...
    def _generate_qvality_input_files(self):
        ...
        for tag in ["target", "decoy"]:
            ...
            self.params["translations"][tag]["path"] = "{0}_{1}_scores.txt".format(
                os.path.join(self.params["output_dir_path"], self.params["file_root"]),
                tag,
            )
        ...
        for spectrum_title, grouped_psm_list in self.params["grouped_psms"].items():
            ...
            min_score = self.UNODE_UPARAMS["validation_minimum_score"][
                "uvalue_style_translation"
            ][self.params["last_engine"]]
            if score_2_write < min_score:
                score_2_write = min_score
            print(score_2_write, file=crazy_tmp_files[tag])
        for tag in ["target", "decoy"]:
