...
class pepnovo_3_1(ursgal.UNode):
    ...
    META_INFO = {
        ...
        "citation": "Ari M. Frank, Mikhail M. Savitski, Michael L. Nielsen, Roman A. "
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.available_mods = {
            ...
            "Deamidated": {"opt": ("NQ", "+1")},
            "Methyl": {"opt": ("CHKNQR", "+14"), "N-term": "^+14"},
            "Phospho": {"opt": ("STY", "+80")},
    ...
    def preflight(self):
        ...
        self.params["translations"]["tmp_output_file_incl_path"] = os.path.join(
            self.params["output_dir_path"], self.params["output_file"] + ".tmp"
        )
        ...
        if self.params["translations"]["denovo_model_dir"] == None:
            self.params["translations"]["denovo_model_dir"] = os.path.join(
                os.path.dirname(self.exe), "Models"
            )
                ...
                if "fix" in self.available_mods[mod["name"]].keys():
                    if mod["aa"] in self.available_mods[mod["name"]]["fix"][0]:
                        modifications.append(
                            mod["aa"] + self.available_mods[mod["name"]]["fix"][1]
                        )
                        mod_available = True
            ...
            if mod["name"] in self.available_mods.keys():
                ...
                if "opt" in self.available_mods[mod["name"]].keys():
                    if mod["aa"] in self.available_mods[mod["name"]]["opt"][0]:
                        modifications.append(
                            mod["aa"] + self.available_mods[mod["name"]]["opt"][1]
                        )
                        mod_available = True
            if mod_available == False:
                print(
        ...
        self.params["command_list"] = [
            ...
            "-file",
            "{mgf_input_file}".format(
                **self.params["translations"]
            ),  # SpectrumFile (*.mzXML, *.mgf, *.ms2)
            "-model",
            "{denovo_model}".format(
                **self.params["translations"]
            ),  # (currently only CID_IT_TRYP is available)
            "-fragment_tolerance",
            "{frag_mass_tolerance}".format(
                **self.params["translations"]
            ),  # ion tolerances (in Da)
            "-pm_tolerance",
            "{precursor_mass_tolerance}".format(
                **self.params["translations"]
            ),  # precursor ion tolerance (in Da)
            "-digest",
            "{enzyme}".format(**self.params["translations"]),
            "-num_solutions",
            "{num_match_spec}".format(**self.params["translations"]),
            "-model_dir",
            "{denovo_model_dir}".format(
                **self.params["translations"]
            ),  # - directory where model files are kept (default ./Models)
            "-PTMs",
            ":".join(
                modifications
            ),  # - separated by a colons (no spaces) e.g., M+16:S+80:N+1
        ]
        if self.params["translations"]["pepnovo_tag_length"]:
            if (
                self.params["translations"]["pepnovo_tag_length"] >= 3
                and self.params["translations"]["pepnovo_tag_length"] <= 6
            ):
                self.params["command_list"].extend(
                    [
                        "-tag_length",
                        "{pepnovo_tag_length}".format(**self.params["translations"]),
                    ]
        ...
        if (
            self.params["translations"]["min_output_score"] >= 0
            and self.params["translations"]["min_output_score"] <= 1.0
        ):
            self.params["command_list"].extend(
                [
                    "-min_filter_prob",
                    "{min_output_score}".format(**self.params["translations"]),
                ]
            )
    ...
    def _execute(self):
        ...
        if proc is not None:
            output_file = open(
                self.params["translations"]["tmp_output_file_incl_path"], "w"
            )
                ...
                if line != "\n" and save_data == True:
                    ...
                    for elem in line_list:
                        if any(c.isalpha() for c in elem) == True:
                            nr_aa = 0
                            # if spectrum_count == 1:
                            seq_index = line_list.index(
                                elem
                            )  # specifying seq_index for the next section, but only one time
                            for char in elem:
                                if char.isalpha():
                                    nr_aa += 1
                            line_list.reverse()
        ...
        for spec_id in result_dict:
            for rnk_list in result_dict[spec_id]:
                ...
                for x, elem in enumerate(seq):
                    if (
                        not elem.isalpha()
                    ):  # takes everything that is not a letter and adds it to mod
                        mod += elem
                        position = seqlist.index(elem)
                        del seqlist[
                            seqlist.index(elem)
                        ]  # deletes everythin that is not a letter from seqlist
                                ...
                                if type(self.available_mods[key][subkey]) == tuple:
                                    if type(moddict[mod]) == list:
                                        for elem in moddict[mod]:
                                            if (
                                                elem[0]
                                                in self.available_mods[key][subkey][0]
                                            ):
            ...
            for rnk_list in result_dict[spec_id]:
                ...
                for PTM in translated_PTMs.keys():
                    if PTM == rnk_list[8]:
                        ...
                        translated_PTMs_new[translated_PTMs[PTM][0]] = (
                            PTM,
                            translated_PTMs[PTM][1],
                        )
        ...
        for x, spec_id in enumerate(id_list):
            for a in range(len(result_dict[spec_id])):
                if len(result_dict[spec_id]) == 1:
                    ...
                    pepnovo_outputfile_new.write(
                        result_dict[spec_id][a][0] + "," + spec_id + ","
                    )
                    for b in range(1, len(result_dict[spec_id][a])):
                        pepnovo_outputfile_new.write(result_dict[spec_id][a][b] + ",")
