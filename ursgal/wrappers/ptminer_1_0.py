...
class ptminer_1_0(ursgal.UNode):
    ...
    META_INFO = {
        ...
        "citation": "An, Z; Zhai, L; Ying, W; Qian, X; Gong, F; Tan, M; Fu, Y. (2019) PTMiner: "
    ...
    def preflight(self):
        ...
        merged_mgf_file = os.path.join(
            os.path.dirname(mgf_input_files[0]), "PTMiner_merged.mgf"
        )
        ...
        ptminer_input = (
            self.params["translations"]["csv_input_file"].strip(".csv") + ".txt"
        )
        selected_columns = []
        # mod pattern
        ...
        with open(ptminer_input, "w", newline="") as new_csvfile:
            ...
            with open(self.params["translations"]["csv_input_file"], "r") as csv_file:
                ...
                last_search_engine_colname = self.params["translations"][
                    "validation_score_field"
                ]
                last_engine_bigger_scores_better = self.params["translations"][
                ...
                for row in csv_reader:
                    ...
                    tmp_dict["ObsMH"] = (
                        float(row["Exp m/z"]) * float(row["Charge"])
                    ) - (ukb.PROTON * (float(row["Charge"]) - 1))
                    ...
                    tmp_dict["Protein Access"] = (
                        row["Protein ID"].split("<|>")[0].split(" ")[0]
                    )
                    # if tmp_dict['Protein Access'] not in self.protein_lookup.keys():
                    #     self.protein_lookup[tmp_dict['Protein Access']] = row['Protein ID']
                    tmp_dict["Before AA"] = (
                        row["Sequence Pre AA"].split("<|>")[0].split(";")[0]
                    )
                    tmp_dict["After AA"] = (
                        row["Sequence Post AA"].split("<|>")[0].split(";")[0]
                    )
        ...
        # params to write
        self.params_to_write["open_search"] = self.params["translations"][
            "is_open_search"
        ]
        self.params_to_write["precursor_matching_tolerance"] = max(
            abs(self.params["translations"]["min_mod_size"]),
...
"""
        ...
        self.params_to_write["precursor_tol"] = max(
            abs(self.params["translations"]["precursor_mass_tolerance_plus"]),
            abs(self.params["translations"]["precursor_mass_tolerance_minus"]),
        )
        self.params_to_write["precursor_tol_type"] = self.params["translations"][
            "precursor_mass_tolerance_unit"
        ]
        self.params_to_write["fragment_tol"] = self.params["translations"][
            "frag_mass_tolerance"
        ]
        self.params_to_write["fragment_tol_type"] = self.params["translations"][
        ...
        self.params_to_write["is_localized"] = self.params["translations"][
            "determine_localization"
        ]
        self.params_to_write["min_mod_number"] = self.params["translations"][
            "min_num_mods"
        ]
        self.params_to_write["use_prior"] = self.params["translations"][
            "use_prior_probability"
        ]
        self.params_to_write["is_annotated"] = self.params["translations"][
            "determine_unimod_annotation"
        ]
        self.params_to_write["protein_database"] = os.path.abspath(
            self.params["translations"]["database"]
        )
        self.params_to_write["output"] = (
            os.path.dirname(self.params["translations"]["output_file_incl_path"]) + "\\"
        )
    ...
    def postflight(self, anno_result=None, csv_input=None, merged_results_csv=None):
        ...
        with open(csv_input, "r") as csvfile:
            ...
            for row in reader:
                psm_identifier = "||".join(
                    [row["Spectrum Title"], row["Sequence"], row["Modifications"]]
                )
        ...
        if anno_result is None:
            filtered_result = os.path.join(
                self.params_to_write["output"], "filtered_result.txt"
            )
            loc_result = os.path.join(self.params_to_write["output"], "loc_result.txt")
            prior_probability = os.path.join(
                self.params_to_write["output"], "prior_probability.txt"
            )
        ...
        with open(anno_result, "r") as anno_file, open(
            merged_results_csv, "w"
        ) as out_file:
            writer = csv.DictWriter(
                out_file, fieldnames=fieldnames, lineterminator=lineterminator
            )
            writer.writeheader()
            ...
            for row in reader:
                ...
                if row["#"] != "*":
                    ...
                    for line_dict in original_rows[psm_identifier]:
                        ...
                        line_dict["PTMiner:Mass Shift Localization"] = ";".join(
                            mass_shift_pos
                        )
                        line_dict[
                            "PTMiner:Posterior Probability"
                    ...
                    if (
                        str(row["New Sequence"]).strip() != ""
                        and row["New Sequence"] is not None
                    ):
                        for line_dict in original_rows[psm_identifier]:
                    ...
                    if unimod_id is None:
                        new_mass_shift_anno = "Unannotated mass-shift {0}".format(
                            annotated_mass
                        )
                    else:
                    ...
                    for line_dict in original_rows[psm_identifier]:
                        ...
                        line_dict["PTMiner:Annotated Mod Pos within Pep or Prot"] = row[
                            "ObsMH"
                        ]
                        line_dict["PTMiner:Annotated Mod Classification"] = row[
                            "Mass Shift"
                        ]
