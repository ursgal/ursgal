...
class omssa_2_1_9(ursgal.UNode):
    ...
    META_INFO = {
        ...
        "citation": "Geer LY, Markey SP, Kowalak JA, Wagner L, Xu M, Maynard DM, "
    ...
    def _load_omssa_xml(self):
        ...
        for xml_name in xml_names:
            omssa_xml = os.path.join(xml_path, xml_name)
            self.print_info(
            ...
            for event, element in xml.etree.ElementTree.iterparse(omssa_xml):
                if element.tag.endswith("MSModSpec_residues_E"):
                    ...
                    if (
                        tmp["omssa_name"]
                        in self.META_INFO["mods_to_unimod_correction"].keys()
                    ):
    ...
    def preflight(self):
        ...
        if blastdb_present is False:
            self.print_info("Executing makeblastdb...", caller="Info")
            proc = subprocess.Popen(
                [
                    os.path.join(os.path.dirname(self.exe), "makeblastdb"),
                    "-in",
    ...
    OMSSA. Continuing without modification: {0}
        ...
        self.params["_tmp_output_file_incl_path"] = os.path.join(
            self.params["output_dir_path"], self.params["output_file"] + "_tmp"
        )
        ...
        translations["-fm"]["mgf_input_file"] = self.params["translations"][
            "mgf_input_file"
        ]
        ...
        for translated_key, translation_dict in translations.items():
            ...
            if translated_key == ("-oc", "-ox"):
                self.params["command_list"].extend(
                    (translated_value, self.params["_tmp_output_file_incl_path"])
                )
                continue
            elif translated_key == "-te":
                self.params["command_list"].extend(
                    (translated_key, str(self.params["_omssa_precursor_error"]))
                )
                continue
