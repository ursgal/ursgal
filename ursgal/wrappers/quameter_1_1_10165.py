...
class quameter_1_1_10165(ursgal.UNode):
    ...
    META_INFO = {
        ...
        "citation": "Ma, Z. Q., Polzin, K. O., Dasari, S., Chambers, M. C., Schilling, B., "
    ...
    def preflight(self):
        ...
        if self.input_file.lower().endswith(".mzml") or self.input_file.lower().endswith(
            ...
            self.params["translations"]["mzml_input_file"] = self.meta_unodes[
                "ucontroller"
            ].get_mzml_that_corresponds_to_mgf(self.input_file)
            self.print_info(
            ...
            raise Exception(
                "QuaMeter input spectrum file must be in mzML or RAW format!"
            )
    ...
    def postflight(self):
        ...
        shutil.move(
            output_file_2_move, self.params["translations"]["output_file_incl_path"]
        )
        return
