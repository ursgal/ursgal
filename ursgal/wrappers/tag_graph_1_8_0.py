...
class tag_graph_1_8_0(ursgal.UNode):
    ...
    META_INFO = {
        ...
        "citation": "Devabhaktuni, A.; Lin, S.; Zhang, L.; Swaminathan, K.; Gonzalez, CG.; Olsson, N.; Pearlman, SM.; Rawson, K.; Elias, JE."
        "(2019) TagGraph reveals vast protein modification landscapes from large tandem mass spectrometry datasets."
        "Nat Biotechnol. 37(4)",
    }
    ...
    def reformat_de_novo_file(self, unified_de_novo_results=None, mod2mass=None):
        ...
        for i, unified_de_novo_result in enumerate(unified_de_novo_results):
            ...
            with open(unified_de_novo_result, "r") as in_file, open(
                reformatted_input, "w"
            ) as out_file:
                csv_reader = csv.DictReader(in_file)
                csv_writer = csv.DictWriter(out_file, fieldnames=new_header)
                ...
                for line_dict in csv_reader:
                    ...
                    for translated_header in header_translations.keys():
                        if translated_header in [
                            ...
                            for n, aa in enumerate(line_dict["Sequence"]):
                                ...
                                if n + 1 in mods.keys():
                                    ...
                                    new_sequence += "{0}({1}{2})".format(
                                        aa, add_plus, round(mass, 2)
                                    )
                                    mod_list.append("{0} ({1})".format(name, aa))
                                else:
                                    new_sequence += aa
                            ...
                            new_line_dict[translated_header] = line_dict[
                                header_translations[translated_header]
                            ]
    ...
    def preflight(self):
        ...
        if self.input_file.lower().endswith(
            ...
            multi_input = True
            self.input_file = [
                os.path.join(d["dir"], d["file"])
                for d in self.params["input_file_dicts"]
            ]
            self.params["translations"]["mzml_input_file"] = self.input_file
        else:
            raise Exception("TagGraph input spectrum file must be in mzML format!")
        ...
        if isinstance(self.params["translations"]["mzml_input_file"], list):
            ...
            shutil.copy(
                self.params["translations"]["mzml_input_file"], self.tag_graph_tmp_dir
            )

        self.docker_dir_path = "/mnt/ursgal_taggraph/"
        self.docker_mount = "{0}:{1}".format(
            self.tag_graph_tmp_dir, self.docker_dir_path
        )
                ...
                if type(taggraph_param_name) is tuple:
                    ...
                    if self.params["translations"]["frag_mass_tolerance_unit"] == "da":
                        self.params_to_write[
                            ...
                            param_value, base_mz=self.params["translations"]["base_mz"]
                        )
                    else:
                    ...
                    if param_value == "default":
                        self.params_to_write[taggraph_param_name] = file_locations[
                            taggraph_param_name
                        ]
                    else:
                        self.params_to_write[taggraph_param_name] = param_value
                    ...
                    self.params_to_write[taggraph_param_name] = os.path.basename(
                        param_value
                    ).replace(".fasta", ".fm")
        ...
        self.params_to_write["de_novo"] = [
            os.path.basename(reformatted_input)
            for reformatted_input in reformatted_inputs
        ]
    ...
    def write_params_file(self):
        with open(self.param_file_name, "w") as io:
            assert len(self.params_to_write["de_novo"]) == len(
                self.params_to_write["mzml_file"]
            ), "Must have same number of denovo results as input files"

            file_input_lines = []
            for denovo, mzml in zip(
                self.params_to_write["de_novo"], self.params_to_write["mzml_file"]
            ):
                file_input_lines.append(
                    "{denovo}|{mzml}".format(denovo=denovo, mzml=mzml)
                )

            with open(
                os.path.join(self.tag_graph_tmp_dir, self.params_to_write["de_novo"][0])
            ) as fin:
                fn = fin.readline().strip().split(",")

            new_name = (
                os.path.splitext(self.params["output_file"])[0]
                + "deepnovo_pointnovo_unified_tmp_denovo_merged.csv"
            )
