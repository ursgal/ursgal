...
class novor_1_1beta(ursgal.UNode):
    ...
    META_INFO = {
        ...
        "citation": "Bin Ma (2015) Novor: Real-Time Peptide de Novo Sequencing "
        "Software.",
    }
    ...
    def preflight(self):
        ...
        self.params["translations"]["params_file"] = os.path.join(
            self.params["output_dir_path"], self.params["output_file"] + "_Params.txt"
        )
        ...
        for mod_type in [potential_mods, fixed_mods]:
            ...
            for mod in not_available_mods.keys():
                if (
                    ...
                    not in available_mods
                ):
                    print(
                        """
            [ WARNING ] Novor does not support your given modification
            [ WARNING ] Continue without modification {0} ({1})""".format(
        ...
        params_file = open(
            self.params["translations"]["params_file"], "w", encoding="UTF-8"
        )
        params2write = [
            ...
            "fragmentIonErrorTol = {frag_mass_tolerance}{frag_mass_tolerance_unit}".format(
                **self.params["translations"]
            ),
            "precursorErrorTol = {precursor_mass_tolerance}{precursor_mass_tolerance_unit}".format(
                **self.params["translations"]
            ),
            "variableModifications = {potential_modifications}".format(
                **self.params["translations"]
            ),
            "fixedModifications = {fixed_modifications}".format(
                **self.params["translations"]
            ),
            "forbiddenResidues = {forbidden_residues}".format(
                **self.params["translations"]
            ),
        ]
        ...
        self.params["command_list"] = [
            self.exe,
            "-p",
            "{params_file}".format(**self.params["translations"]),
    ...
    def postflight(self):
        ...
        for line_dict in csv_dict_reader_object:
                ...
                if header_translations[k] == "Sequence":
                    ...
                    for regex_pattern, unimod_name in regex_list:
                        formated_peptide = ursgal.ucore.reformat_peptide(
                            regex_pattern, unimod_name, formated_peptide
                        )
            ...
            new_line_dict["Raw data location"] = self.params["translations"][
                "mgf_input_file"
            ]
        ...
        csv_dict_writer_object = csv.DictWriter(
            new_result_file, fieldnames=translated_headers, **csv_kwargs
        )
