...
class msamanda_1_0_0_5243(ursgal.UNode):
    ...
    META_INFO = {
        ...
        "citation": "Dorfer V, Pichler P, Stranzl T, Stadlmann J, Taus T, Winkler S, "
    ...
    def preflight(self):
        ...
        self.params["command_list"] += [
            ...
            "{0}".format(
                self.params["translations"]["output_file_incl_path"] + "_settings.xml"
            ),
        ...
        self.params["translations"]["instruments_file_input"] = "".join(
            instruments_file_input
        )
        ...
        (
            self.params["translations"]["enzyme_cleavage"],
            self.params["translations"]["enzyme_position"],
            self.params["translations"]["enzyme_inhibitors"],
        ) = self.params["translations"]["enzyme"].split(";")
        ...
        for file_name, content in templates.items():
            with open(
                self.params["translations"]["output_file_incl_path"] + file_name, "w"
            ) as out:
                print(content, file=out)
                self.print_info("Wrote input file {0}".format(file_name), caller="Info")
    ...
    def postflight(self):
        ...
        with open(
            self.params["translations"]["output_file_incl_path"], "r"
        ) as result_file:
            ...
            translated_headers += ["Is decoy", "Start", "Stop", "Calc m/z"]
            self.print_info("Loading unformatted MS Amanda results ...", caller="Info")
            for line_dict in csv_dict_reader_object:
                cached_msamanada_output.append(line_dict)
        self.print_info(
            "Loading unformatted MS Amanda results done", caller="postflight"
        )
        ...
        with open(
            ...
            csv_dict_writer_object = csv.DictWriter(
                result_file, fieldnames=translated_headers
            )
            csv_dict_writer_object.writeheader()
            self.print_info(
                "Writing MS Amanda results, this can take a while...", caller="Info"
            )
            csv_write_list = []
        ...
        self.print_info("Writing MS Amanda results done!", caller="Info")
        pass
