...
class uninovo_alpha_052013(ursgal.UNode):
    ...
    META_INFO = {
        ...
        "citation": "Jeong K, Kim S, Pevzner PA (2013): UniNovo: a universal tool for "
    ...
    def preflight(self):
        ...
        self.params["mgf_input_file"] = os.path.join(
            self.params["input_dir_path"], self.params["file_root"] + ".mgf"
        )
        ...
        self.params["precursor_mass_tolerance"] = (
        ...
        self.params["command_list"] = [
            "pushd",
            self.exe.rstrip("UniNovo.jar"),
            "&&",  # need to specify this because UniNovo searches the Pars directory in the shell working directory
            "java",
            "-Xmx{java_-Xmx}".format(**self.params),
            "-jar",
            ...
            "-o",
            "{output_file_incl_path}".format(**self.params).rstrip(
                ".den"
            ),  # output file prefix - UniNovo will output file named as "prefix.den"
            "-pt",
            "{precursor_mass_tolerance}{precursor_mass_tolerance_unit}".format(
                **self.params
            ),  # precursor ion tolerance (ending in ppm or Da)
            "-t",
            "{frag_mass_tolerance}{frag_mass_tolerance_unit}".format(
                **self.params
            ),  # ion tolerances (ending in ppm or Da)
            "-f",
            "{frag_method}".format(
                **self.params
            ),  # fragmentation method (CID/ETD/HCD) - if -par option is specified, this option will be ignored
            "-e",
            "{enzyme}".format(
                **self.params
            ),  # enzyme applied (0: No enzyme specificity, 1: Trypsin (default), 2: LysC)
            "-c",
            "{uninovo_num_13C}".format(
                **self.params
            ),  # number of 13C considered (default : 0)
            ...
            "-n",
            "{num_match_spec}".format(
                **self.params
            ),  # number of de novo sequences per one spectrum (1-200 : default 100)
            "-acc",
            "{uninovo_accuracy}".format(
                **self.params
            ),  # set accuracy threshold (0.0-0.9 : default 0.8)
            "-g",
            "{uninovo_num_mass_gaps}".format(
                **self.params
            ),  # number of possible mass gaps per each sequence (2-10 : default 10)
  ...
  alternatively, redefine uNode._execute() in the engine class altogether
        ...
        if len(self.params["command_list"]) != 0:
            proc = subprocess.Popen(
                self.params["command_list"], stdout=subprocess.PIPE, shell=True
            )
        else:
