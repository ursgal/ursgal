...
class msgfplus_v2019_07_03(ursgal.UNode):
    """
    MSGF+ UNode
        ...
        "engine": {
            "platform_independent": {
                "arc_independent": {
                    ...
                    "zip_md5": "9367333d34662d53eb5ace7f4948ba93",
                    "additional_exe": [],
                },
            },
        },
        "citation": "Kim S, Mischerikow N, Bandeira N, Navarro JD, Wich L, "
    ...
    def __init__(self, *args, **kwargs):
        super(msgfplus_v2019_07_03, self).__init__(*args, **kwargs)
        pass
    ...
    def preflight(self):
        ...
        enzyme_txt_path = os.path.join(os.getcwd(), "params", "enzymes.txt")
        ...
        for line in lines:
            if line.startswith("CHARGE"):
                ...
                print(
                    "ACTIVATIONMETHOD={0}".format(frag_method), file=mgf_new_input_file
                )
            else:
                print(line, file=mgf_new_input_file)
        mgf_new_input_file.close()

        translations["-s"]["mgf_input_file"] = self.params["translations"][
            "mgf_new_input_file"
        ]
        ...
        self.params["translations"]["modification_file"] = os.path.join(
            self.params["output_dir_path"], self.params["output_file"] + "_Mods.txt"
        )
        ...
        mods_file = open(
            self.params["translations"]["modification_file"], "w", encoding="UTF-8"
        )
        modifications = []

        print(
            "NumMods={0}".format(translations["NumMods"]["max_num_mods"]),
            file=mods_file,
        )
        ...
        for t in ["fix", "opt"]:
            for mod in self.params["mods"][t]:
                modifications.append(
                    "{0},{1},{2},{3},{4}".format(
                        mod["mass"], mod["aa"], t, mod["pos"], mod["name"]
                    )
                )
