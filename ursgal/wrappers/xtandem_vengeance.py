...
class xtandem_vengeance(ursgal.UNode):
    ...
    META_INFO = {
        ...
        "citation": "Craig R, Beavis RC. (2004) TANDEM: matching proteins with tandem "
        "mass spectra.",
    }
    ...
    def preflight(self):
        ...
        for file_name in xml_required:
            ...
            xml_file_path = (
                self.params["translations"]["output_file_incl_path"].strip(".xml")
                + file_name
            )
...
[ WARNING ] Continue without modification {0} """.format(
        ...
        for file_name, content in templates.items():
            ...
            with open(xml_file_path, "w") as out:
                ...
                self.print_info(
                    "Wrote input file {0}".format(file_name), caller="preflight"
                )
    ...
    def format_templates(self):
        ...
        """
        templates = {
            "15N-masses.xml": """<?xml version="1.0"?>
