...
class percolator_2_08(ursgal.UNode):
    ...
    META_INFO = {
        ...
        "citation": "Käll L, Canterbury JD, Weston J, Noble WS, MacCoss MJ. (2007) "
            ...
            for m, (score, line_dict) in enumerate(
                ...
                if cleavage_site == "C":
                    for i, aa in enumerate(pre_aa):
                        ...
                        if line_dict["Sequence"][-1] in allowed_aa or "-" in post_aa:
                            t["enzC"] = 1
    ...
    def postflight(self):
        ...
        """
        percolator_log_file_name = self.params["translations"][
            "output_file_incl_path"
        ].replace(".csv", "_percolator_log.txt")
        percolator_log_file = open(percolator_log_file_name, "w")
        ...
        for title, count in sorted(percolator_stats.items()):
            self.print_info("{0} : {1}".format(title, count), caller="Info")
        self.print_info(
