...
class compomics_utilities_4_11_5(ursgal.UNode):
    ...
    META_INFO = {
        ...
        "citation": "Barsnes, H., Vaudel, M., Colaert, N., Helsens, K., Sickmann, A.,"
        "Berven, F. S., and Martens, L. (2011) compomics-utilities: an "
        "open-source Java library for computational proteomics. BMC"
        "Bioinformatics 12, 70",
    }
    ...
    def preflight(self):
        ...
        if (
            ...
            self.tmp_peptide_file = os.path.join(
                self.params["output_dir_path"], "peptides.txt"
            )
            with open(self.tmp_peptide_file, "w") as io:
                ...
                for line_dict in csv.DictReader(open(input_file_incl_path, "r")):
                    ...
                    for aa_to_replace, replace_dict in sorted(
                        self.params["translations"]["aa_exception_dict"].items(),
                        reverse=True,
                    ):
    ...
    def postflight(self):
        ...
        for fastaID, sequence in ursgal.ucore.parse_fasta(
            open(self.params["translations"]["database"], "r")
        ):
            fasta_id_dict[fastaID] = sequence
            try:
                short_2_long_name_mapping[
                    fastaID.split("|")[1]
                ] = fastaID  # uniprot style
            except:
                pass
