...
class crux_2_1(ursgal.UNode):
    ...
    def print_param_line(self, key, value, io):
        """"""
        print("{0}={1}".format(key, value), file=io)
        return

    def write_param_file(self):
        self.params["param_file"] = (
            self.params["output_file_basename_incl_path"] + "_crux_params.txt"
        )
        ...
        for mod in self.params["mods"]["fix"]:
            if (
                self.params["label"] == "15N"
                and mod["aa"] == "C"
                and mod["name"] == "Carbamidomethyl"
            ):
                cam = True
                continue
        ...
        for aminoacid, modification in ursgal.ukb.DICT_15N_DIFF.items():
            if aminoacid == "C":
                if cam == True:
                    ...
                    modification = 0
                print_param_line(aminoacid, modification, io)
    ...
    def preflight(self):
        ...
        self.params["output_file"] = self.params[
            "output_file_basename_incl_path"
        ] + "{0}.csv".format(self.params["ident_csv_suffix"])

        self.params["fileroot"] = os.path.basename(
            self.params["output_file_basename_incl_path"]
        )
        ...
        self.params["command_list"] = [
            ...
            "--overwrite",
            "T",  # Replace existing files if true (T) or fail when trying to overwrite a file if false (F)
        ...
        self.params["command_list"] = [
            ...
            # '--mzid-output', 'T',
            "--concat",
            "T",  # report target and decoy hits
            "--parameter-file",  # A file containing command-line or additional parameters
            "{param_file}".format(**self.params),
            "--output-dir",  # The name of the directory where output files will be created
            "{output_folder}".format(**self.params),
