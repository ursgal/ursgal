...
class myrimatch_2_1_138(ursgal.UNode):
    ...
    META_INFO = {
        ...
        "citation": "Tabb DL, Fernando CG, Chambers MC. (2007) MyriMatch: highly "
    ...
    def preflight(self):
        ...
        self.params["translations"]["myrimatch_ions_to_search"] = "manual:" + ",".join(
            self.params["translations"]["myrimatch_ions_to_search"]
        )

        self.params["translations"][
            "myrimatch_config_file_path"
        ] = "{output_file_incl_path}_myrimatch_params.cfg".format(
            **self.params["translations"]
        )
        ...
        self.params["command_list"] = [
            self.exe,
            "-cfg",
            "{myrimatch_config_file_path}".format(**self.params["translations"]),
            "{mzml_input_file}".format(**self.params["translations"]),
            "-workdir",
            "{output_dir_path}".format(**self.params),
            "-cpus",
            "{cpus}".format(**self.params["translations"]),
            # '-dump'
        ]
    ...
    def write_param_file(self):
        ...
        config_file = open(
            self.params["translations"]["myrimatch_config_file_path"], "w"
        )  # param file must be in execution folder
        ...
        if self.params["translations"]["label"] == "15N":
            for aminoacid, N15_Diff in ursgal.ukb.DICT_15N_DIFF.items():
                ...
                if existing == True:
                    continue
                self.params["translations"][
                    "myrimatch_static_mods"
                ] += "{0} {1} ".format(aminoacid, N15_Diff)

        for mod in self.params["mods"]["fix"]:
            self.params["translations"]["myrimatch_static_mods"] += "{0} {1} ".format(
                mod["aa"], mod["mass"]
            )
        ...
        for n, mod in enumerate(self.params["mods"]["opt"]):
            if n >= len(characters) - 1:
                print(
                    ...
                    [ WARNING ] Continues without modification: {0}""".format(
                        mod
                    )
                )
                continue
            if mod["pos"] != "any":
                ...
                continue
            self.params["translations"][
                "myrimatch_dynamic_mods"
            ] += "{0} {1} {2} ".format(mod["aa"], characters[n], mod["mass"])
        ...
        self.params["translations"]["database"] = os.path.abspath(
    ...
    def postflight(self):
        ...
        if self.params["prefix"] is None:
            out_file_root = os.path.join(
                self.params["output_dir_path"], self.params["file_root"]
            )
        else:
            out_file_root = os.path.join(
                self.params["output_dir_path"],
                self.params["file_root"].replace(self.params["prefix"] + "_", ""),
            )
        ...
        for possible_outfile in possible_outfiles:
            if os.path.exists(possible_outfile):
                os.rename(possible_outfile, desired_output_name)
                break
