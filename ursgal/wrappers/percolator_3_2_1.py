...
class percolator_3_2_1(ursgal.UNode):
    ...
    def preflight(self):
        ...
        PERCOLATOR_FIELDS = OrderedDict(
            [
                ...
                ("Label", {"csv_field": "", "DefaultDirection": "-"}),
                ("ScanNr", {"csv_field": "Spectrum ID", "DefaultDirection": "-"}),
                (
                    "lnrSp",
                    {
                        "csv_field": "",
                        "DefaultDirection": 0,
                        "description": "The natural logarithm of the rank of the match based on the Sp score",
                    },
                ),
                (
                    "deltLCn",
                    {
                        "csv_field": "",
                        "DefaultDirection": 0,
                        "description": "The difference between this PSM's XCorr and the XCorr of the last-ranked \
                ...
                (
                    "deltCn",
                    {
                        "csv_field": "",
                        "DefaultDirection": 0,
                        "description": "The difference between this PSM's XCorr and the XCorr of the next-ranked \
                PSM for this spectrum, divided by this PSM's XCorr or 1, whichever is larger. \
                ...
                (
                    "Xcorr",
                    {
                        "csv_field": "",
                        "DefaultDirection": 0,
                        "description": "The SEQUEST cross-correlation score",
                    },
                ),
                (
                    "Sp",
                    {
                        "csv_field": "",
                        "DefaultDirection": 0,
                        "description": "The preliminary SEQUEST score.",
                    },
                ),
                (
                    "IonFrac",
                    {
                        "csv_field": "",
                        "DefaultDirection": 0,
                        "description": "The fraction of b and y ions theoretical ions matched to the spectrum",
                    },
                ),
                (
                    "Mass",
                    {
                        "csv_field": "",
                        "DefaultDirection": 0,
                        "description": "The observed mass [M+H]+",
                    },
                ),
                (
                    "PepLen",
                    {
                        "csv_field": "",
                        "DefaultDirection": 0,
                        "description": "The length of the matched peptide, in residues",
                    },
                ),
                ("Charge1", {"csv_field": "", "DefaultDirection": 0}),
                ("Charge2", {"csv_field": "", "DefaultDirection": 0}),
                ("Charge3", {"csv_field": "", "DefaultDirection": 0}),
                ("Charge4", {"csv_field": "", "DefaultDirection": 0}),
                ("Charge5", {"csv_field": "", "DefaultDirection": 0}),
                ("Charge6", {"csv_field": "", "DefaultDirection": 0}),
                ("Charge7", {"csv_field": "", "DefaultDirection": 0}),
                ("Charge8", {"csv_field": "", "DefaultDirection": 0}),
                ("Charge9", {"csv_field": "", "DefaultDirection": 0}),
                ("Charge10", {"csv_field": "", "DefaultDirection": 0}),
                (
                    "enzN",
                    {
                        "csv_field": "",
                ...
                (
                    "Peptide",
                    {"csv_field": "", "DefaultDirection": 0, "description": ""},
                ),
                (
                    "Proteins",
                    {
        ...
        self.params["translations"]["decoy_output_file_incl_path"] = os.path.join(
            self.params["output_dir_path"], "decoysOnly_" + self.params["output_file"]
        )

        self.params["translations"][
            "percolator_in"
        ] = "{output_file_incl_path}.tsv".format(**self.params["translations"])
        ...
        writer = csv.DictWriter(
            o, list(PERCOLATOR_FIELDS.keys()), delimiter="\t", extrasaction="ignore"
        )
        writer.writeheader()

        self.params["translations"][
            "percolator_decoy_out"
        ...
        if self.params["translations"]["percolator_post_processing"] == "mix-max":
            self.params["command_list"].append("-y")
        ...
        last_search_engine = self.get_last_search_engine(history=self.stats["history"])
        ...
        for n, spectrum_title in enumerate(self.params["grouped_psms"].keys()):
            ...
            for m, (score, line_dict) in enumerate(
                ...
                pre_aa = []
                for prot_pre_aa in line_dict["Sequence Pre AA"].split(
                    self.params["translations"]["protein_delimiter"]
                ):
                    for p_pre_aa in prot_pre_aa.split(";"):
                        pre_aa.append(p_pre_aa)
                post_aa = []
                for prot_post_aa in line_dict["Sequence Post AA"].split(
                    self.params["translations"]["protein_delimiter"]
                ):
                    for p_post_aa in prot_post_aa.split(";"):
                        post_aa.append(p_post_aa)
                allowed_aa = set(
                    self.params["translations"]["enzyme"].split(";")[0] + "-"
                )
                ...
                if cleavage_site == "C":
                    for i, aa in enumerate(pre_aa):
                        if aa in allowed_aa or line_dict["Sequence Start"] in [
                            "1",
                            "2",
                        ]:
                            t["enzN"] = 1
                        ...
                        if line_dict["Sequence"][-1] in allowed_aa or post_aa == "-":
                            t["enzC"] = 1
                    ...
                    for i, aa in enumerate(post_aa):
                        ...
                        if line_dict["Sequence"][0] in allowed_aa or line_dict[
                            "Sequence Start"
                        ] in ["1", "2"]:
                            t["enzN"] = 1
                ...
                if mods.strip() == "":
                    t["Peptide"] = "{0}.{Sequence}.{1}".format(
                        sorted(pre_aa)[0], sorted(post_aa)[0], **line_dict
                    )
                else:
                    t["Peptide"] = "{0}.{Sequence}#{1}.{2}".format(
                        sorted(pre_aa)[0], mods, sorted(post_aa)[0], **line_dict
                    )
    ...
    def postflight(self):
        ...
        potential_buggy_percolator_output = (
            self.params["translations"]["percolator_out"] + ".psms"
        )
        ...
        for line_dict in csv_input:
            ...
            if self.params["infer_proteins"] is True:
                line_dict["Protein q-value"] = protein_fdr_lookup.get(
                    line_dict["Protein ID"].replace(" ", "_"), {"q-value": ""}
                )["q-value"]
                line_dict["Protein PEP"] = protein_fdr_lookup.get(
                    line_dict["Protein ID"].replace(" ", "_"), {"PEP": ""}
                )["PEP"]
                line_dict["Protein Group"] = protein_fdr_lookup.get(
                    line_dict["Protein ID"].replace(" ", "_"), {"ID": ""}
                )
            ...
            if _psmid_pep_key in s2l[psm_type].keys():
                ...
                line_dict["Percolator:SVM Score"] = s2l[psm_type][_psmid_pep_key][
                    "score"
                ]
