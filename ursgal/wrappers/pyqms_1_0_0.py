...
class pyqms_1_0_0(ursgal.UNode):
    ...
    def _execute(self):
        ...
        if self.params["translations"]["ms_level"] == 1:
            ...
            self.params["translations"]["REL_MZ_RANGE"] = self.params["translations"][
                "frag_mass_tolerance"
            ]
        ...
        self.params["translations"]["REL_MZ_RANGE"] = (
            self.params["translations"]["REL_MZ_RANGE"] * 1e-6
        )

        pyqms_params = {
            "PERCENTILE_FORMAT_STRING": self.params["translations"][
                "label_percentile_format_string"
            ],
            "M_SCORE_THRESHOLD": self.params["translations"]["m_score_cutoff"],
            "ELEMENT_MIN_ABUNDANCE": self.params["translations"][
                "min_element_abundance"
            ],
            "MIN_REL_PEAK_INTENSITY_FOR_MATCHING": self.params["translations"][
                "pyqms_min_rel_isotope_peak_intensity"
            ],
            "REQUIRED_PERCENTILE_PEAK_OVERLAP": self.params["translations"][
                "required_percentile_peak_overlap"
            ],
            "MINIMUM_NUMBER_OF_MATCHED_ISOTOPOLOGUES": self.params["translations"][
                "min_number_of_matched_isotopologues"
            ],
            "INTENSITY_TRANSFORMATION_FACTOR": self.params["translations"][
                "intensity_transformation_factor"
            ],
            "UPPER_MZ_LIMIT": self.params["translations"]["upper_mz_limit"],
            "LOWER_MZ_LIMIT": self.params["translations"]["lower_mz_limit"],
            "MZ_TRANSFORMATION_FACTOR": self.params["translations"][
                "mz_transformation_factor"
            ],
            "REL_MZ_RANGE": self.params["translations"]["REL_MZ_RANGE"],
            "REL_I_RANGE": self.params["translations"]["rel_intensity_error"],
            "INTERNAL_PRECISION": self.params["translations"]["internal_precision"],
            "MAX_MOLECULES_PER_MATCH_BIN": self.params["translations"][
                "max_molecules_per_match_bin"
            ],
            "SILAC_AAS_LOCKED_IN_EXPERIMENT": self.params["translations"][
                "silac_aas_locked_in_experiment"
            ],
            "BUILD_RESULT_INDEX": self.params["translations"][
                "build_pyqms_result_index"
            ],
            "MACHINE_OFFSET_IN_PPM": self.params["translations"][
                "machine_offset_in_ppm"
            ],
            "FIXED_LABEL_ISOTOPE_ENRICHMENT_LEVELS": self.params["translations"][
        ...
        for fixed_mod in self.params["mods"]["fix"]:
            ...
            fixed_labels[aa].append(
                {"element_composition": comp, "evidence_mod_name": name}
            )
