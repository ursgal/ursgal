...
class UController(ursgal.UNode):
    ...
    def _collect_all_unode_wrappers(self):
        ...
        for wrapper_file in glob.glob(wrappers_path_glob):
            ...
            distributable = wrapper_meta_info.get("distributable", True)
            style = wrapper_meta_info.get("utranslation_style", None)
            assert (
                style is not None
            ), """
            ...
            if wrapper_module_name in engine_2_style.keys():
                assert (
                    ...
                    {2}
                """.format(
                    wrapper_module_name, engine_2_style[wrapper_module_name], style
                )
            else:
                engine_2_style[wrapper_module_name] = style
            ...
            uses_unode = wrapper_meta_info.get("uses_unode", None)
            ...
            for meta_type, meta_type_bool in engine_type.items():
                if meta_type_bool is True:
                    ...
                    unodes["__by_meta_type"][meta_type].append(wrapper_module_name)
        return unodes
    ...
    def convert_results_to_csv(self, input_file, force=None, output_file_name=None):
        ...
        if input_suffix == ".csv":
            ...
            if "xtandem" in last_engine:
                ...
                engine_name = self.params.get("msgfplus_mzid_converter_version", None)
                if engine_name is None:
                    engine_name = self.UNODE_UPARAMS["msgfplus_mzid_converter_version"][
    ...
    def convert_to_mgf_and_update_rt_lookup(
        self, input_file, force=None, output_file_name=None
    ):
        """
        ...
        if os.path.exists(scan_rt_lookup_path) is False:
            ...
            scan_rt_lookup = pickle.load(open(scan_rt_lookup_path, "rb"))
        ...
        if answer is None:
            ...
            if file_root not in scan_rt_lookup.keys():
                answer = "{file_root} has no entry in RT lookup pkl: {0}".format(
                    scan_rt_lookup_path, **self.io["output"]["finfo"]
                )

        report = self.run_unode_if_required(force, engine_name, answer)
        ...
        if answer is not None:
            ...
            with open(scan_rt_lookup_path, "wb") as rtpkl:
                pickle.dump(self.scan_rt_lookup, rtpkl)
    ...
    def determine_availability_of_unodes(self):
        ...
        arc_specific_engine_folders = os.path.join(
            ursgal.base_dir, "resources", self.platform, self.architecture[0]
        )
        arc_independent_engine_folders = os.path.join(
            ursgal.base_dir, "resources", "platform_independent", "arc_independent"
        )
        engine_folders = [
        ...
        for platform_key, arc_key, engine_folder in engine_folders:
            ...
            for engine in sorted(self.unodes.keys()):
                ...
                exe_name = kb_engine_entry[platform_key][arc_key].get("exe", None)
                if exe_name is None:
                    continue
    ...
    def engine_sanity_check(self, short_engine):
      ...
      Multiple hits found: {1}.
        ...
        assert (
            ...
            or self.unodes[full_engine_name].get("in_development", False) is True
        ), """
      Requested engine {0} was mapped on {1}, which is not available
      on your system.
        """.format(
            short_engine, full_engine_name
        )
    ...
    def fetch_file(self, engine=None):
        ...
        assert (
            engine is not None
        ), """Please specify a download engine
        ...
        try:
            if self.params["prefix"] is not None:
                tmp_file_name = "{0}_{1}".format(
                    self.params["prefix"], self.io["input"]["finfo"]["json"]
                )
            else:
            ...
            self.print_info(
                "Automatic removal of tmp file {0} failed".format(path_2_remove),
                caller="Info",
            )
            self.print_info(
    ...
    def generate_target_decoy(
        self, input_files=None, engine=None, force=False, output_file_name=None
    ):
        """
        ...
        """
        self.print_old_function_warning("generate_target_decoy", "execute_misc_engine")
        if engine is None:
            engine = "generate_target_decoy_1_0_0"
    ...
    def merge_csvs(
        self, input_files, force=None, output_file_name=None, merge_duplicates=False
    ):
        """
    ...
    def combine_search_results(
        self, input_files, engine=None, force=None, output_file_name=None
    ):
        """
        ...
        answer = self.prepare_unode_run(
            input_files, output_file=output_file_name, engine=engine_name, force=force
        )

        # making sure that all input files are from different engines:
        input_file_dicts = self.io["output"]["params"]["input_file_dicts"]
        input_file_search_engines = {ifd["last_engine"] for ifd in input_file_dicts}

        assert len(input_file_search_engines) == len(
            input_files
        ), """
      ...
      *different* search engines.
      ...
      merge them beforehand using merge_csvs().""".format(
            len(input_files), input_files, len(input_file_search_engines)
        )
    ...
    def prepare_unode_run(self, input_file, engine=None, force=None, output_file=None):
        ...
        self.set_ios(
            input_file, engine=engine_name, userdefined_output_fname=output_file
        )
    ...
    def dump_multi_json(self, fpath, fdicts):
        ...
        keys_to_delete = self.params.get("del_from_params_before_json_dump", [])
        ...
        with open(json_file, "w") as file_object:
            json.dump(j_content, file_object, sort_keys=True, indent=2)
            ...
            if os.path.exists(file_json_path):
                ...
                if len(json_content) > 3 and "history" in json_content[3]:
                    ...
                    if last_engine is not None and type(last_engine) != list:
                        ...
                        last_engine_colname = self.UNODE_UPARAMS[
                            "validation_score_field"
                        ]["uvalue_style_translation"][last_engine]
                ...
                file_dict.update(self.set_file_info_dict(input_file))
    ...
    def set_ios(self, input_file, engine=None, userdefined_output_fname=None):
        ...
        #
        self.print_info(msg='Setting self.io["input"]', caller="set_ios")
        ...
        if self.io["input"]["finfo"]["json_exists"] is False:
            ...
            for u_param, u_value in self.params.items():
                if (
                    u_param not in self.io["input"]["params"]
                    or self.io["input"]["params"][u_param] != u_value
                ):
                    #
            ...
            for i_json_param, i_json_value in self.io["input"]["params"].items():
                ...
                if i_json_param not in self.meta_unodes[engine].PARAMS_TRIGGERING_RERUN:
                    continue
                default_value = self.meta_unodes[engine].UNODE_UPARAMS[i_json_param][
                    "default_value"
                ]
                if i_json_value != default_value:
                    ...
                    self.print_info(
                        "Mismatch of param {0}: UController: {1}; i_json: {2}".format(
                            i_json_param, default_value, i_json_value
                        )
                    )
        ...
        # setting pickle path ...
        pickle_path = self.io["input"].get("scan_rt_lookup_path", None)
    ...
    def sanitize_userdefined_output_filename(self, user_fname, engine):
        ...
        self.print_info(
            "You defined {0}'s output file name as: {1}".format(engine, out_fname),
            caller="Info",
        )
        return out_fname
    ...
    def take_care_of_params_and_stats(self, io_mode=None):
        ...
        if self.io[io_mode]["finfo"]["json_exists"]:
            j_content = self.load_json(finfo=self.io[io_mode]["finfo"])
    ...
    def build_name_from_history_or_regen(self, engine):
        ...
        file_name_blocks.append(self.io["input"]["finfo"]["file_root"].strip(".u"))
        ...
        output_suffix = self.meta_unodes[engine].META_INFO.get("output_suffix", engine)
        ...
        if file_extension is None:
            self.print_info(
                ...
                "kb.{0}.META_INFO. Keeping input file extension".format(self.engine),
                caller="WARNING",
            )
        ...
        if self.params.get("compress_raw_search_results_if_possible", False):
            if self.UNODE_UPARAMS["compress_raw_search_results_if_possible"][
                "uvalue_style_translation"
            ].get(engine, False):
                output_file += ".gz"

        path_building_blocks = [self.io["input"]["finfo"]["dir"]]
        ...
        if (
            engine_creates_folder == True
            and self.params["engines_create_folders"] == True
        ):

            engine_folder = os.path.join(self.io["input"]["finfo"]["dir"], engine)
            if os.path.exists(engine_folder) is False:
                os.mkdir(engine_folder, mode=0o777)
            path_building_blocks.append(engine_folder)

        path_building_blocks.append(output_file)
        full_output = os.path.join(*path_building_blocks)
        self.print_info(
            "Generated engine {0} output file name: {1}".format(engine, full_output),
            caller="Info",
        )
        return full_output
    ...
    def eval_if_run_needs_to_be_executed(self, engine=None, force=None):
        ...
        if self.io["output"]["finfo"]["json_exists"]:
            ...
            for used_param in self.meta_unodes[engine].PARAMS_TRIGGERING_RERUN:
                if used_param in o_params.keys() and used_param in i_params.keys():
                    ...
                    default_value = self.meta_unodes[engine].UNODE_UPARAMS[used_param][
                        "default_value"
                    ]
        ...
        if self.io["output"]["finfo"]["json_exists"]:
            ...
            if len(o_history) > 0:
                last_o_history = self.io["output"]["stats"]["history"][-1]
                # print( o_history )
                # print( last_o_history )
                # print( 'history overview ')
                if last_o_history["status"] != "complete":
                    reasons.append(
                        "Previous run was not completed, status {status}".format(
                            **last_o_history
                        )
                    )
        if (
    ...
    def compress_out_if_possible(self, raw_search_results, engine):
        global_ucontroller_compress_flag = self.params.get(
            "compress_raw_search_results_if_possible", False
        )
        compress_engine_output = self.unodes[engine]["class"].META_INFO.get(
        ...
        print(
            ...
            """.format(
                compress_engine_output, global_ucontroller_compress_flag
            )
        )
        if compress_engine_output and global_ucontroller_compress_flag:
    ...
    def search_mgf(
        self, input_file, engine=None, force=None, output_file_name=None, multi=False
    ):
        """
        ...
        for search_engine_type in [
            ...
            if (
                self.unodes[engine_name]["class"].META_INFO.get(
    ...
    def search(
        ...
        engine_type = (
            self.unodes[engine_name[0]]["class"]
            .META_INFO["engine_type"]
            .get("protein_database_search_engine", False)
        )
    ...
    def quantify(
        self, input_file, engine, force=None, output_file_name=None, multi=False
    ):
        """
        ...
        self.input_file_sanity_check(
            input_file, extensions=["mzml", "mzml.gz", "mzml.igz"], multi=multi
        )
    ...
    def get_mzml_that_corresponds_to_mgf(self, mgf_path):
        ...
        """
        error_msg = (
            "\n  Could not find the mzML file that " "corresponds to {0}"
        ).format(mgf_path)
        if hasattr(self, "io"):
            self.io["tmp"] = {}
        else:
        ...
        for entry in history:
            if entry["engine"] == self.params["mzml2mgf_converter_version"]:
                corresponding_mzml = os.path.join(
                    entry["finfo"]["dir"], entry["finfo"]["file"]
                )
                break
        else:
    ...
    def set_profile(self, profile, dev_mode=False):
        ...
        number_of_updated_params = len(ursgal.PROFILES[profile])
        self.print_info("Initializing profile {0}".format(profile), caller="profile")
        self.print_info(
    ...
    def show_unode_overview(self):
        ...
        for meta_type in sorted(self.unodes["__by_meta_type"].keys()):
            ...
            print("\t{BOLD}{0}{ENDC}(s):".format(meta_type.upper(), **ursgal.COLORS))
            for engine in sorted(self.unodes["__by_meta_type"][meta_type]):

                if self.unodes[engine]["in_development"]:
                    continue

                node_status = self.unodes[engine].get("import_status", "n/a")
                ...
                print(
                    "\t{0: >3} : {1:28} [{3} {2: ^25} {ENDC}] {4}".format(
                        n, engine, node_status, color, url_available, **ursgal.COLORS
                    )
                )
                n += 1
        return

    def run_unode_if_required(
        self, force, engine_name, answer, merge_duplicates=False, history_addon=None
    ):
        """
        ...
        if answer is None:
            ...
            self.print_info("Reason for run: {0}".format(answer), caller="Info")
                ...
                if psm_counter != Counter():
                    if max(psm_counter.values()) > 1:
                        out_file = ursgal.ucore.merge_duplicate_psm_rows(
                            ...
                            psm_defining_colnames=psm_defining_colnames,
                            psm_colnames_to_merge_multiple_values=self.params[
                                "psm_colnames_to_merge_multiple_values"
                            ],
                        )
                        self.print_info(
                            "Result file path: {0}".format(out_file), caller="Info"
                        )

            self.dump_json_and_calc_md5(stats=report["stats"], params=report["params"])

        self.verify_engine_produced_an_output_file(report["output_file"], engine_name)
    ...
    def unify_csv(self, input_file, force=False, output_file_name=None):
        ...
        self.print_old_function_warning("unify_csv", "execute_misc_engine")
    ...
    def map_peptides_to_fasta(self, input_file, force=False, output_file_name=None):
        ...
        Returns:
            str: Path of the output file
        """
        self.print_old_function_warning("map_peptides_to_fasta", "execute_misc_engine")
    ...
    def filter_csv(self, input_file, force=False, output_file_name=None):
        ...
        """
        self.print_old_function_warning("filter_csv", "execute_misc_engine")
        return self.execute_unode(
            input_file=input_file,
    ...
    def prepare_resources(self, root_zip_target_folder):
        ...
        for root_dir, folder_list, file_list in os.walk(root_resource_folder):
            ...
            if engine in self.unodes.keys():
                ...
                include_in_git = self.unodes[engine].get("include_in_git", None)
                if include_in_git in [None, True]:
                    continue
                # or we cannot dictribute it, restrictive licenses etc.
                distributable = self.unodes[engine].get("distributable", True)
                ...
                zip_file_name_for_shutil = os.path.join(
                    target_folder_for_zips, "{0}".format(engine)
                )
                zip_file_name = "{0}.zip".format(zip_file_name_for_shutil)

                md5_file_name = os.path.join(
                    target_folder_for_zips, "{0}.md".format(engine)
                )
                kb_engine_dict = self.unodes[engine].get("engine", None)
                if kb_engine_dict is None:
                    ...
                    continue
                try:
                    md5_in_kb = kb_engine_dict[current_platform][current_architecture][
                        "zip_md5"
                    ]
                    kb_has_md5 = True
                except:
        ...
        Nothing to do here...
                        ...
                        shutil.make_archive(
                            zip_file_name_for_shutil, "zip", resource_source_folder
                        )
                        ...
                        if calculated_zip_md5 == md5_in_kb:
                            ...
                            zip_file_list.append((zip_file_name, calculated_zip_md5))

                        else:
                            print(
                                "md5 in kb found for {0} on platform {1} and architecture {2} differs from your zip file.".format(
                                    engine, current_platform, current_architecture
                                ),
                            ...
                            print(
                                "created zip file has been removed, include correct md5 in META_IFO first"
                            )
                else:
                    print(
    ...
    def download_resources(self, resources=None):
        ...
        for engine in resources:
            if "resource_folder" in self.unodes[engine].keys():
                ...
                if self.unodes[engine]["available"] is False:
                    ...
                    try:
                        md5_in_kb = kb_engine_dict[current_platform][
                            current_architecture
                        ]["zip_md5"]
                    except:
                        print(
                    ...
                    if os.path.exists(self.unodes[engine]["resource_folder"]) is False:
                        os.makedirs(
                            os.path.join(
                                ursgal_folder, self.unodes[engine]["resource_folder"]
                            )
                        )
        ...
        Please install the engine manually!
                    ...
                    if md5_in_kb == online_md_5:
                        ...
                        tmp_http_get_params["http_url"] = os.path.join(
                            self.params["ursgal_resource_url"], zip_file_name
                        ).replace("\\", "/")
                        print(
        ...
        [  INFO   ] Please contact the Ursgal team!
                        ...
                        # check md5 of zip!!!
                        calculated_zip_md5 = self.calc_md5(
                            os.path.join(os.path.dirname(ursgal_folder), zip_file_name)
                        )
                        if calculated_zip_md5 != md5_in_kb:
                            print(
                                """
        ...
        [  INFO   ] Please contact the Ursgal team!
                            ...
                            os.remove(
                                os.path.join(
                                    os.path.dirname(ursgal_folder), zip_file_name
                                )
                            )
                            continue

                        download_zip_files.append((engine, zip_file_name))
                        executables_list = [
                            self.unodes[engine]["engine"][current_platform][
                                current_architecture
                            ]["exe"]
                        ]
                        executables_list += self.unodes[engine]["engine"][
                            current_platform
                        ][current_architecture].get("additional_exe", [])
                        with zipfile.ZipFile(
                            ...
                            for name in io.namelist():
                                ...
                                target_file = os.path.join(
                                    tmp_http_get_params["http_output_folder"], name
                                )
...
>>>
        ...
        report = self.run_unode_if_required(
            force, engine_name, answer, merge_duplicates=merge_duplicates
        )
        return report["output_file"]
    ...
    def visualize(
        self, input_files, engine=None, force=None, output_file_name=None, multi=True
    ):
        """
    ...
    def input_file_sanity_check(
        self, input_file, engine=None, extensions=None, multi=False, custom_str=None
    ):
        """
        ...
        if custom_str is None:
            custom_str = "Input to {0}()".format(caller_function_name)
        else:
            custom_str = "Error in {0}() - ".format(caller_function_name) + custom_str
        ...
        if multi:
            ...
            assert (
                isinstance(input_file, list) and len(input_file) >= 2
            ), """
        ...
        \t{input_file}""".format(
            ...
            input_file_list = [
                input_file
            ]  # put it into a list, so we can treat both str and list inputs the same (loop over them)
        ...
        \t{in_file}""".format(
            ...
            if engine is not None:
                engine_extensions = self.unodes[engine]["class"].META_INFO[
                    "input_extensions"
                ]
...
class ParamsDict(dict):
    ...
    def update(self, dict_to_add):
        ...
        if unknown_params:
            ...
            raise ValueError(
                "Unknown UController parameter(s): {}. "
                "Please check your spelling, and check "
                "http://ursgal.readthedocs.io/en/latest/parameter.html "
