...
def main(
    input_file=None, output_file=None, filter_rules=None, output_file_unfiltered=None
):
    """
    Filters csvs
    """
    ...
    if output_file_unfiltered is not None:
        unfiltered_output_file_object = open(output_file_unfiltered, "w")

    with open(input_file, "r") as in_file:
        csv_input = csv.DictReader(in_file)
        csv_output = csv.DictWriter(
            output_file_object, list(csv_input.fieldnames), **csv_kwargs
        )
        csv_output.writeheader()
        if output_file_unfiltered is not None:
            unfiltered_csv_output = csv.DictWriter(
                unfiltered_output_file_object, list(csv_input.fieldnames), **csv_kwargs
            )
...
Did you misspell the field name?""".format(
                    ...
                    if rule in ["lte", "gte", "lt", "gt"]:
                        ...
                        if re.search(value, line_dict[dict_key]) is not None:
                            ...
                            line_dict[dict_key][-2] == "N"
                            and line_dict[dict_key][-1] != "P"
                        ):
                            if (
                                "S" in line_dict["Sequence Post AA"]
                                or "T" in line_dict["Sequence Post AA"]
                            ):
                                write_row_bools.add(True)
                            else:
                                write_row_bools.add(False)
