...
def main(
    ...
    assert result_file is not None or plot_molecule_dict is not None, '''
    ...
    for pt in plot_types:
        if pt not in [
            ...
            assert validated_results_pkl is not None, '''
    ...
    if result_file is not None and plot_types != ['check_peak_presence']:
        assert plot_molecule_dict is None or plot_molecule_dict == {}, '''
    ...
    if 'check_frag_specs' in plot_types:
        ursgal_output_file, venn_data1, sugarpy_output_file, venn_data2 = sp_results.check_frag_specs(
            mzml_file=mzml_file,
