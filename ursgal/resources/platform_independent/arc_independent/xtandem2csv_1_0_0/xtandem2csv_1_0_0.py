...
def main(input_file=None, decoy_tag=None, output_file=None):
    ...
    if input_file.endswith(".gz"):
        ...
        #     gzip.open(path)
        # )
        file_object = codecs.getreader("utf-8")(gzip.open(input_file))
    else:
        file_object = codecs.open(input_file, mode="r", encoding="utf-8")
        ...
        if event == b"start":
            ...
            if element.tag.endswith("group"):
                if "mh" in element.attrib.keys():
                    ...
                    group["Exp m/z"] = (
                        float(element.attrib["mh"]) / float(group["Charge"])
                    ) + PROTON
