...
def main(
    ...
    unmutableTPSet = (
        set()
    )  # has to be on pep level since we add some later during ranking
    mutableTPSet = set()

    def generateMergedFastaKey(IDList):
        """Format the fastaID in the right way """
        if len(IDList) > 1:
            key = "{0}_NumberOfIdenticalSequences_{1}".format(IDList[0], len(IDList))
        else:
    ...
    def generateConvertedSequence(sequence, convert_aa_in_motif):
        ...
        for start, end in matches:
            ...
            new_sequence += (
                sequence[start : start + int(position)]
                + new_aa
    ...
    for sequence, fastaIDList in sequenceFastaDict.items():
        ...
        if counter % 100 == 0:
            print(
                "Processing sequence #{0}/{1}".format(
                    counter, len(sequenceFastaDict.keys())
                ),
                end="\r",
                file=sys.stderr,
            )
        ...
        if mode == "reverse_protein":
            ...
            final_output[fastaID] = {
                "new_sequence_target": "{0}".format(sequence_without_terminator),
                "new_sequence_decoy": "{0}".format(sequence_without_terminator[::-1]),
            }
        elif mode == "shuffle_peptide":
            peptideList = ursgal.ucore.digest(
                sequence, (cleavage_aa, site), no_missed_cleavages=True
            )
            fastaDict[fastaID] = {
                "seqLen": len(sequence),
                ...
                if len(peptide) < 8:
                    if perDict[aaString]["permutated"] is False:
                        for permutation in itertools.permutations(
                            peptideWithoutCleavageAA, len(peptideWithoutCleavageAA)
                        ):
                    ...
                    if perDict[aaString]["mutable"]:
                        ...
                        while a:
                            ...
                            if shuffled_peptide != peptide and check is True:
                                if (
                                    shuffled_peptide
                                    not in perDict[aaString]["permutations"]
                                ):
    ...
    if mode == "shuffle_peptide":
        for aaString in perDict.keys():
            if len(perDict[aaString]["permutations"]) >= len(
                perDict[aaString]["peptides"]
            ):
                for peptide, fastaID in perDict[aaString]["pepProtTuples"]:
                    randomPos = random.randrange(len(perDict[aaString]["permutations"]))
                    if peptide not in mutableTPSet:
                        goodLookUp[peptide] = perDict[aaString]["permutations"][
                            randomPos
                        ]
                    mutableTPSet.add(peptide)
                    fastaDict[fastaID]["lengthOfDecoyablePetides"] += len(aaString)
        ...
        for aaString in perDict.keys():
            if (
                ...
                < len(perDict[aaString]["peptides"])
            ):
                tmp = sorted(
                    [
                        (
                            fastaDict[fastaID]["seqLen"]
                            / fastaDict[fastaID]["lengthOfDecoyablePetides"],
                            fastaID,
                            peptide,
                        )
                        for peptide, fastaID in perDict[aaString]["pepProtTuples"]
                ...
                for n, (coverage, fastaID, peptide) in enumerate(tmp):
                    if n >= len(perDict[aaString]["permutations"]):
                        ...
                        mutableTPSet.add(peptide)
                        goodLookUp[peptide] = perDict[aaString]["permutations"][n]
                        fastaDict[fastaID]["lengthOfDecoyablePetides"] += len(aaString)
        print(
        ...
        print(
            "Writing unmutable peptides in {0}".format(unmutable_peptides),
            file=sys.stderr,
        )

        with open(unmutable_peptides, "w") as io:
            for v, k in sorted([(v, k) for k, v in c.items()], reverse=True):
                print("{0: >10}\t{1}".format(v, k), file=io)

    print(file=sys.stderr)
    # NOTE the reduncdancy printout:
    print("=== Number of NON UNIQUE protein models with the same sequence ===")
    print(
        "{0} These were merged to {1} new protein model names".format(
            len(listNonUniqueModels), len(nonUniqueModels)
        ),
        file=sys.stderr,
    )

    print("=== Number of UNIQUE protein models ===")
    print("{0}".format(len(uniqueModels)), file=sys.stderr)
    ...
    for n, (fastaID, sequence_dict) in enumerate(final_output.items()):
        if n % 100 == 0:
            print(
                "Writing new sequence #{0}/{1}".format(n, total_sequences_2_write),
                end="\r",
                file=sys.stderr,
            )
        ...
        print(
            ">{0}{1}".format(decoy_tag, fastaID),
            file=opened_output_file,
            end=line_ending,
        )
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        ...
        name_of_db = main(
            sys.argv[1:], output_file, enzyme=("RK;C;P"), mode="shuffle_peptide"
        )
        print(name_of_db)
