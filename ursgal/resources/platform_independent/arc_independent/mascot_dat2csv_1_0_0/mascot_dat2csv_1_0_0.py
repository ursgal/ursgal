...
class MascotDatParser(object):
    def __init__(self, mascot_dat_file):
        ...
        self.peptides = self._create_final_lookup(self.peptide_dict, self.section_dict)
        ...
        for line in section_dict["peptides"]:
            ...
            if len(k_splitted) == 2:
                ...
                if "terms" in k:
                    ...
                    q, p, subst = k.split("_")
                    if (
                        pep_info["_{0}_{1}".format(q, p)].get("substitution", None)
                        is None
                    ):
        ...
        for query_with_rank in pep_info:
            ...
            if query_dict["title"].startswith("msmsid"):
                ...
                if m is not None:
                    ...
                    query_dict["retention_time"] = float(rtinminutes) / 60.0
                    charge = query_dict["charge"].replace("+", "")
                    query_dict["title"] = "{0}.{1}.{1}.{2}".format(
                        os.path.splitext(os.path.basename(self.search_file))[0],
                        spec_id,
                        charge,
                    )
    ...
    def _format_mods(self, pepdict):
        ...
        for pos, mod in enumerate(pepdict["var_mod_string"]):
            ...
            if var_mod_string is not None:
                ...
                mods.append("{mod}:{pos}".format(mod=var_mod, pos=pos))
            ...
            if fix_mod_string is not None:
                ...
                mods.append("{mod}:{pos}".format(mod=fix_mod, pos=pos))
        return mods
