...
def main(
    ...
    print(
        "sanitize removed {0} PSMs for {1} spectra".format(
            len(removed_psms), len(conflicting_specs)
        )
    )
    ...
    csv_output = csv.DictWriter(output_file_object, list(fieldnames), **csv_kwargs)
...
def group_psms(input_file, validation_score_field=None, bigger_scores_better=None):
    ...
    if validation_score_field is None:
        ...
        assert (
            "multiple engines:" not in search_engine
    ...
    for spectrum_title in grouped_psms.keys():
        grouped_psms[spectrum_title].sort(
            key=operator.itemgetter(0), reverse=bigger_scores_better
        )
    print(
