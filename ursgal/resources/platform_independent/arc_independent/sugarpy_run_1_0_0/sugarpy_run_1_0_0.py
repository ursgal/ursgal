...
def main(
    ...
    assert (
        ident_file is not None
    ), """
    [Error] ident_file is required """
    assert (
        mzml_file is not None
    ), """
    ...
    assert (
        monosaccharides is not None
    ), """
    [Error] No monosaccharide compositions are given """

    sp_run = sugarpy.run.Run(monosaccharides=monosaccharides)
    ...
    if output_file is None:
        output_file = "{0}_sugarpy.csv".format(ident_file.replace(".csv", ""))
    ...
    for n, peptide_unimod in enumerate(sorted(peps_with_glycans)):
        ...
        rt_min = min(peptide_lookup[peptide_unimod]["rt"]) - rt_border_tolerance
        rt_max = max(peptide_lookup[peptide_unimod]["rt"]) + rt_border_tolerance
        if use_median_accuracy == "peptide":
            ppm_offset = statistics.median(peptide_lookup[peptide_unimod]["accuracy"])
        pyqms_params["MACHINE_OFFSET_IN_PPM"] = ppm_offset
        print(
            "[ SugarPy  ] Quantification for peptide ",
            peptide_unimod,
            "#{0} out of {1}".format(n + 1, len(peps_with_glycans)),
        )
        results_pkl = sp_run.quantify(
            ...
            pkl_name=os.path.join(
                output_folder,
                "{0}_{1}_pyQms_results.pkl".format(mzml_basename, pkl_name),
            ),
            mzml_file=mzml_file,
            force=force,
            # spectra=[],
    ...
    sp_results = sugarpy.results.Results(
        validated_results=validated_results, monosaccharides=monosaccharides
    )
