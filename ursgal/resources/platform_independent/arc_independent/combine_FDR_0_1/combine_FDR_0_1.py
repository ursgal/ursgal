...
class MultiScorer(object):
    ...
    def __init__(self, csv_path, info=None):
        MultiScorer.csv_instances.append(
            self
        )  # add this class instance to the list of instances
    ...
    def get_shared_PSM_rowlist(cls, csv_instances, other_csv_instances):
        ...
        PSM_tuple_sets = [set(eng.rowdict.keys()) for eng in csv_instances]
    ...
    def add_FDR_score(cls, idents):
        ...
        for i, row in enumerate(idents):
            ...
            if q_val != q_val_prev or i == len(idents) - 1:
                ...
                intercept, gradient = calc_intercept_and_slope(
    ...
    def add_combined_FDR_score(cls, shared_PSMs):
        ...
        qval_sorted_csv = sorted(
            shared_PSMs, key=lambda x: (float(x["q-Value"]), x["AFS"])
        )  # sort csv by q-value (AFS if ties)
    ...
    def current_engine_combination_status_str(cls, used_engines, other_engines):
        ...
        if len(other_engines) == 0:
            return "Scoring PSMs that were identified by all {0} engines...".format(
                len(used_engines)
            )
        else:
            return "Scoring PSMs that were identified by {0}, but not by {1}...".format(
                engines_string, other_engines_string
            )
    ...
    def write_summary_csv(cls, full_file_path):
        ...
        header_end = ["Is decoy", "Protein ID"]
        header_center = []
        ...
        for raw_row in cls.summary_output:
            ...
            for instance in cls.csv_instances:
                if PSM in instance.rowdict:
                    ...
                    row[instance.engine_colname + ":FDR"] = instance.rowdict[PSM][
                        "Estimated FDR"
                    ]
                    for key in header:
        ...
        sorted_output_row_list = sorted(
            output_row_list,
            key=lambda x: (
                float(x["Combined FDR Score"]),
                float(x["Average FDR Score"]),
            ),
        )
...
def main(
    ...
    for csv_instance in MultiScorer.csv_instances:
        ...
        for row in ident_csv:
            csv_instance.rowdict[row["PSM tuple"]]["FDR Score"] = row["FDR Score"]
    ...
    for engine_combo in all_combinations(MultiScorer.csv_instances):
        ...
        status_string = MultiScorer.current_engine_combination_status_str(
            engine_combo, other_engines
        )
        print(status_string, end="\r")

        # peptide identifications made by a particular combination of search engines:
        shared_PSMs_unordered = MultiScorer.get_shared_PSM_rowlist(
            engine_combo, other_engines
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "directory", help="the directory in which to look for '*complete_v.csv'-files"
    )
    parser.add_argument(
        "-C",
        "--cutoff",
        default=0.01,
        help="the desired combined FDR cutoff, as a fraction",
        type=float,
    )
    parser.add_argument(
        "-L",
    ...
    if not found_csv:
        print(
            "Error: No CSV file that ends in '*complete_v.csv' and contains label identifier '{0}' was found in directory '{1}'.".format(
                args.label, args.directory
            )
        )
