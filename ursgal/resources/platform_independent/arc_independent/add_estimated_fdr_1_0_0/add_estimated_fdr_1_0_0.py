...
def main(
    input_file=None, output_file=None, score_colname=None, bigger_scores_better=False
):

    assert input_file is not None, "add_estimated_FDR requires an input file!"
    assert (
        score_colname is not None
    ), "add_estimated_FDR needs to know the quality score column's name!"
    ...
    with open(input_file, "r") as in_csv:
        with open(output_file, "w", newline="") as out_csv:
            ...
            assert (
                "estimated_FDR" not in reader.fieldnames
            ), """
  Input file {0} to add_estimated_fdr already has a column called "estimated_FDR"!
            ...
            print(
                """Sorting {0} by column "{1}" ...""".format(input_file, score_colname)
            )

            sorted_csv = sorted(
                reader,
                key=lambda d: float(d[score_colname]),
