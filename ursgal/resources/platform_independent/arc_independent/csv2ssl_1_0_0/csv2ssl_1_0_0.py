...
def main(input_file=None, output_file=None, score_column_name=None, score_type=None):
    ...
    with open(input_file, "r") as in_file:
        ...
        csv_output = csv.DictWriter(
            output_file_object, new_fieldnames, delimiter="\t", **csv_kwargs
        )
        ...
        for csv_line_dict in csv_input:
            ...
            for mod in mods:
                ...
                if ":" not in mod:
                    sys.exit(
                        "This unimod: {0} requires positional information".format(mod)
                    )
                for occ, match in enumerate(pattern.finditer(mod)):
                    ...
                    if occ >= 1:
                        sys.exit("Incorrect regex pattern for mod: {0}".format(mod))
