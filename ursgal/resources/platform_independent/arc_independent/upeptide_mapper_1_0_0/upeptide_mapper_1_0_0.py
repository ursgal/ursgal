...
def main(input_file=None, output_file=None, params=None):
    ...
    csv_file_buffer, tmp_peptide_set, output_fieldnames = parse_input_file(
        input_file=input_file, params=params
    )
    print(
        """[ map_peps ] Mapping {0} peptides using mapper {1}""".format(
            len(tmp_peptide_set), params["translations"]["peptide_mapper_class_version"]
        )
    )
    p2p_mappings = upapa.map_peptides(list(tmp_peptide_set), *class_etxra_args)
    ...
    if len(non_mappable_peps) != 0:
        print(
            ...
            [ WARNING ] Check Search and Database if neccesary""".format(
                ...
                if len(list(non_mappable_peps)) < 100
                else list(non_mappable_peps)[:99],
        ...
        # reset buffer...
        print(
            "[ map_peps ] Checking for I <--> L conversions: mapping {0} variants".format(
                len(tmp_peptide_set)
            )
        )
        ...
        (
            still_non_mappable_peps,
            still_non_mappable_linedicts,
            new_td_peps,
            new_dnd,
        ) = write_output_file(
            output_file=output_file,
        ...
        if len(still_non_mappable_peps) != 0:
            ...
            print(
                "[ map_peps ] Checking for X in sequence: mapping {0} variants".format(
                    len(tmp_peptide_set)
                )
            )
            if len(tmp_peptide_set) == 0:
                ...
                p2p_mappings = upapa.map_peptides(
                    list(tmp_peptide_set), *class_etxra_args
                )

            (
                final_non_mappable_peps,
                final_non_mappable_linedicts,
                new_td_peps,
                new_dnd,
            ) = write_output_file(
                output_file=output_file,
            ...
            print(
                ...
                [ WARNING ] {2}""".format(
                    ...
                    if len(list(final_non_mappable_peps)) < 100
                    else list(final_non_mappable_peps)[:99],
                )
            )
    ...
    if len(target_decoy_peps) != 0:
        print(
            ...
            [ WARNING ] 'Is decoy' has been set to 'True' """.format(
                target_decoy_peps
                if len(target_decoy_peps) < 100
                else list(target_decoy_peps)[:99],
...
def parse_input_file(input_file=None, params=None):
    ...
    with open(input_file, "r") as in_file:
        ...
        for line_dict in csv_input:
            ...
            appended = False
            for aa_to_replace, replace_dict in sorted(
                params["translations"]["aa_exception_dict"].items(), reverse=True
            ):
                if aa_to_replace in line_dict["Sequence"]:
                    if aa_to_replace in ["O"]:
                        if "unimod_name" in replace_dict.keys():
                            for r_pos, aa in enumerate(line_dict["Sequence"]):
                                if aa == aa_to_replace:
                                    ...
                                    new_mod = "{0}:{1}".format(unimod_name, index_of_aa)
                        ...
                        product_combos = []
                        new_peptides = set()
                        for combo_tuple in itertools.product(
                            replace_dict["original_aa"], repeat=repeat_count
                        ):
                            ...
                            for pos, new_aa in enumerate(combo_tuple):
                                ...
                                tmp_peptide_set.add(remapped_peptide)
...
[ WARNING ] Please adjust upeptide_mapper accordingly""".format(
                                aa_to_replace
                            )
                        )
                        sys.exit(1)
...
def write_output_file(
    ...
    with open(output_file, write_type) as output_file_object, open(
        output_file + "_full_protein_names.txt", write_type
    ) as protein_id_output:
        ...
        for line_nr, line_dict in enumerate(csv_file_buffer):
            ...
            for seq_variant in seq_var_list:
                lookup_identifier = "{0}><{1}".format(
                    seq_variant, params["fasta_lookup_name"]
                )
                if lookup_identifier not in pep_map_lookup.keys():
                    ...
                    sorted_upeptide_maps = [
                        protein_dict
                        for protein_dict in sorted(upeptide_maps, key=lambda x: x["id"])
                    ]
                    ...
                    for protein in sorted_upeptide_maps:
                        if protein_mapping_dict is None:
                            ...
                            if protein["id"] == last_protein_id:
                                ...
                                protein_mapping_dict["Protein ID"] += "{0}{1}".format(
                                    tmp_join_char, protein["id"]
                                )

                            protein_mapping_dict["Sequence Start"] += "{0}{1}".format(
                                tmp_join_char, str(protein["start"])
                            )
                            protein_mapping_dict["Sequence Stop"] += "{0}{1}".format(
                                tmp_join_char, str(protein["end"])
                            )
                            protein_mapping_dict["Sequence Pre AA"] += "{0}{1}".format(
                                tmp_join_char, protein["pre"]
                            )
                            protein_mapping_dict["Sequence Post AA"] += "{0}{1}".format(
                    ...
                    if len(protein_mapping_dict["Protein ID"]) >= 2000:
                        ...
                        protein_mapping_dict["Protein ID"] = (
                            protein_mapping_dict["Protein ID"][:1990] + " ..."
                        )
                        do_not_delete = True
...
[ WARNING ] This PSM will be skipped.
                    """.format(
                        org_sequence, params["fasta_lookup_name"], ""
                    )
                )
...
class UPeptideMapper_v2(dict):
    ...
    def build_lookup(self, fasta_name=None, fasta_stream=None, force=True):
        ...
        """
        print(
            "[ upapa v2 ] UPeptideMapper is building the fast cache and regular sequence dict from a fasta"
        )
...
class UPeptideMapper_v3:
    ...
    def cache_database(self, fasta_database, fasta_name):
        ...
        for protein_pos, (protein_id, seq) in enumerate(
            ursgal.ucore.parse_fasta(open(fasta_database, "r").readlines())
        ):
            if protein_pos % 5000 == 0:
                print(
                    "[ upapa v3 ] Buffering protein #{0} of database {1}".format(
                        self.fasta_counter[fasta_name], os.path.basename(fasta_database)
                    ),
                    end="\r",
                )
            len_seq = len(seq)
        ...
        self.total_sequence_string[fasta_name] = "".join(
            self.total_sequence_list[fasta_name]
        )
    ...
    def map_peptides(self, peptide_list, fasta_name):
        ...
        for match in self.automatons[fasta_name].iter(
            self.total_sequence_string[fasta_name]
        ):
            idx, (p_idx, m_peptide) = match
            len_m_peptide = len(m_peptide)
            protein_name_end_index = self.protein_list[fasta_name][idx]
            protein_name_start_index = self.protein_list[fasta_name][
                idx - len_m_peptide + 1
            ]
            if protein_name_end_index != protein_name_start_index:
                ...
                # sys.exit(1)
                continue
            protein_start_in_sequence_string = self.protein_indices[fasta_name][
                protein_name_end_index
            ]["start"]
            protein_stop_in_sequence_string = self.protein_indices[fasta_name][
                protein_name_end_index
            ]["stop"]
