...
TEST_DICT = {
    "enzyme": {
        ...
        "ukey_translation": {"omssa_style_1": "-ot", "xtandem_style_1": "enzyme"},
        "uvalue_translation": {
            "omssa_style_1": {"trypsin": 1},
            "xtandem_style_1": {"trypsin": "[KR]|{P}"},
        },
    },
    "frag_mass_type": {
        ...
        "uvalue_translation": {"omssa_style": {"monoisotopic": 0, "average": 1}},
    },
}
...
class UMapMaster(unittest.TestCase):
    ...
    def test_all_mappings_by_style(self):
        ...
        self.assertEqual(
            sorted(results),
            sorted([("enzyme", "trypsin"), ("frag_mass_type", "monoisotopic")]),
        )

    def test_get_masked_params(self):
        mask = ["default_value", "description"]
        masked_params = self.umama_small.get_masked_params(mask=mask)
        self.assertEqual(len(masked_params.keys()), len(TEST_DICT.keys()))
        self.assertEqual(len(masked_params["enzyme"].keys()), len(mask))
        self.assertEqual(list(sorted(masked_params["enzyme"].keys())), sorted(mask))
