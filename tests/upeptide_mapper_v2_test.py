...
class UMapMaster(unittest.TestCase):
    def setUp(self):
        ...
        upapa_class = self.uc.unodes["upeptide_mapper_1_0_0"][
            "class"
        ].import_engine_as_python_function("UPeptideMapper_v2")
    ...
    def test_peptide_lt_word_len(self):
        ...
        for mapping in maps:
            if mapping["id"] == "Protein1":
                self.assertEqual(expected["Protein1"], mapping)
            if mapping["id"] == "ugly_fasta":
                self.assertEqual(expected["ugly_fasta"], mapping)
    ...
    def test_purge_fasta(self):
        self.upapa_class.purge_fasta_info("Test.fasta")
        self.assertFalse("Test.fasta" in self.upapa_class.keys())

    def test_peptide_gt_word_but_not_continous(self):
        self.assertEqual(
            self.upapa_class.map_peptide(peptide="WHYELVIS", fasta_name="Test.fasta"),
            [],
        )

    def test_multiple_occurrence_in_one_seq(self):
        maps = self.upapa_class.map_peptide(
            peptide="AAAAAAAAAA", fasta_name="Test.fasta"
        )
        self.assertEqual(len(maps), 2)
    ...
    def test_multiple_occurrence_with_opverlap_in_one_seq(self):
        maps = self.upapa_class.map_peptide(peptide="GGGGGGG", fasta_name="Test.fasta")
        self.assertEqual(len(maps), 4)
        ...
        if regex_module_installed:
            ...
            maps = self.upapa_class.map_peptide(peptide="GGGG", fasta_name="Test.fasta")
            self.assertEqual(len(maps), 7)

    def test_sort_independece(self):
        map_1 = self.upapa_class.map_peptide(peptide="FORWARD", fasta_name="Test.fasta")
        map_2 = self.upapa_class.map_peptide(peptide="DRAWROF", fasta_name="Test.fasta")
        map_3 = self.upapa_class.map_peptide(peptide="FORWA", fasta_name="Test.fasta")
        map_4 = self.upapa_class.map_peptide(peptide="AWROF", fasta_name="Test.fasta")
        map_5 = self.upapa_class.map_peptide(
            peptide="AORRW", fasta_name="Test.fasta"
        )  # should not work..
