...
R = ursgal.UController(
    ...
    params={"database": os.path.join("data", "BSA.fasta")},
    force=False,
)

xtandem2csv_main = R.unodes["xtandem2csv_1_0_0"][
    "class"
].import_engine_as_python_function()
input_xml = os.path.join(
    "tests", "data", "xtandem_sledgehammer", "test_BSA1_xtandem.xml"
)
output_csv = os.path.join(
    "tests", "data", "xtandem_sledgehammer", "test_BSA1_xtandem.csv"
)
expected_csv = os.path.join(
    "tests", "data", "xtandem_sledgehammer", "test_BSA1_xtandem_expected.csv"
)
xtandem2csv_main(
...
def unify_xtandem(test_dict, expected_dict):
    for key in [
        ...
        assert test_value == expected_value, print(test_value, expected_value)
