...
class UMapMaster(unittest.TestCase):
    ...
    def setUp(self):
        ...
        upapa_class = self.uc.unodes["upeptide_mapper_1_0_0"][
            "class"
        ].import_engine_as_python_function("UPeptideMapper_v3")
        self.database_path = os.path.join("tests", "data", "Test.fasta")
        with open(self.database_path, "w") as io:
            for line in TEST_FASTA:
                print(line.strip(), file=io)
        # print(upapa_class)
        self.upapa_class = upapa_class(fasta_database=self.database_path)
        return
    ...
    def test_peptide_mapping_1(self):
        self.upapa_class.map_peptides(["ELVIS"], "Test.fasta")
        self.assertEqual(
            [
                (d["id"], d["start"])
        ...
        self.upapa_class.map_peptides(["MUSTACHIO"], "Test.fasta")
        self.assertEqual(
            [
                (d["id"], d["start"])
                for d in self.upapa_class.peptide_2_protein_mappings["Test.fasta"][
    ...
    def test_incremental_cache_buildups(self):
        """"""
        self.upapa_class.cache_database(self.database_path, "Test.fasta")
    ...
    def test_peptide_gt_word_but_not_continous(self):
        self.assertEqual(
            self.upapa_class.map_peptides(["WHYELVIS"], "Test.fasta")["WHYELVIS"], []
        )
        return
    ...
    def test_multiple_occurrence_with_opverlap_in_one_seq(self):
        maps = self.upapa_class.map_peptides(["GGGGGGG"], "Test.fasta")["GGGGGGG"]
        # print(maps)
        self.assertEqual(len(maps), 4)
        self.assertEqual(sorted([m["start"] for m in maps]), [1, 2, 3, 4])
        maps = self.upapa_class.map_peptides(["GGGG"], "Test.fasta")["GGGG"]
        # print(maps)
    ...
    def test_overlap_produces_no_match(self):
        maps = self.upapa_class.map_peptides(["LAPOVER"], "Test.fasta")["LAPOVER"]
    ...
    def test_start_and_end_of_sequence(self):
        maps = self.upapa_class.map_peptides(["OVERLAP"], "Test.fasta")["OVERLAP"]
        # print(maps)
    ...
    def test_end_of_sequence_and_fasta(self):
        maps = self.upapa_class.map_peptides(["LAP"], "Test.fasta")["LAP"]
        # print(maps)
        for map_dict in maps:
            self.assertEqual(map_dict["post"], "-")
        return
