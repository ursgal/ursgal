...
param_info = {
    ...
    "uvalue_type": {"essential": True, "types": [str]},
}
...
def chk_dict_keys(parent_name="", key_list=[], dict_item={}):
    for k in key_list:
        if dict_item.get(k) is None and dict_item.get(k, "None") == "None":
            parent_info = ""
            if parent_name != "":
...
def chk_json_item(
    parent_name="", essential=False, key_list=[], dict_item={}, type_list=[]
):
    ...
    for key_name in key_list:
        if (
            dict_item.get(key_name) is not None
            or dict_item.get(key_name) is None
            and dict_item.get(key_name, "None") != "None"
        ):
        ...
        for k, v in dict_val_search.items():
            if v is not None:
                ...
                if layer_list != [] and layer_list != title_layer_list:
                    error_msg = str(
                        k
                    ) + " has different layer number " "compared with dict_title. {0} vs {1}".format(
                        layer_list, title_layer_list
                    )
                    raise ValueError(error_msg)
