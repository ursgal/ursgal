...
def write_ursgal_pkl_from_mascot_dat(mascot_data_file):
    ...
    pkl_path = os.path.join(os.path.dirname(mascot_data_file), f"_ursgal_lookup.pkl")
                ...
                try:
                    ...
                    for _id, pattern in regexs_for_non_standard_mgfs.items():
                        ...
                        if m is not None:
                            ...
                            query_dict["rtinseconds"] = float(
                                m.group("rt_start_in_seconds")
                            )
...
def main(input_file, database):
    ...
    converted_unfiltered = uc.execute_unode(input_file, engine="mascot_dat2csv")
    converted = uc.execute_misc_engine(
        input_file=converted_unfiltered, engine="filter_csv"
    )
    mapped = uc.map_peptides_to_fasta(converted)
