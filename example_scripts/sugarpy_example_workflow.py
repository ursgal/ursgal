...
def main(folder=None, target_decoy_database=None):
    ...
    for spec_file in mzML_files:
        mgf_file = uc.convert(input_file=spec_file, engine="mzml2mgf_2_0_0")
        results_one_file = []
        for search_engine in search_engines:
            # search MS2 specs for peptides with HexNAc and HexNAc(2) modification
            for mod in ["N,opt,any,HexNAc", "N,opt,any,HexNAc(2)"]:
                uc.params["modifications"].append(mod)
                uc.params["prefix"] = mod.split(",")[3]
                search_result = uc.search_mgf(input_file=mgf_file, engine=search_engine)
                uc.params["prefix"] = ""
                ...
                mapped_results = uc.execute_misc_engine(
                    input_file=converted_result, engine="upeptide_mapper"
                )
        ...
        uc.params["prefix"] = "{0}".format(os.path.basename(spec_file).strip(".mzML"))
