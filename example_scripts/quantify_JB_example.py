...
def main():
    ...
    params = {
        ...
        "ftp_output_folder": os.path.join(
            os.pardir, "example_data", "quantify_JB_example"
        ),
    ...
    mgf = uc.convert(input_file=mzML_file, engine="mzml2mgf_1_0_0")
    ...
    for label in ["14N", "15N"]:
        ...
        for engine in engines:
            res = uc.search(input_file=mgf, engine=engine)
        ...
        fil = uc.filter_csv(input_file=combined_results)
        all_res.append(fil)
    ...
    uc.quantify(input_file=mzML_file, engine="pyqms_1_0_0", force=False)
